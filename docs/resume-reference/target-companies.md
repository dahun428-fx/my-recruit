...
- Risks or gaps:
  ...
  - [갭 인터뷰 완료 2026-08-04] Java 실무 기간: 소유자 확인 — 내담 초기 20개월(2020.10~2022.05)에 Java/Spring 실무가 있었음(세부 미제공, experience-bank 미적재·이력서 미기재 유지). **[2026-09-13 소유자 정정 — 철회]** 초기 20개월은 한국미스미(시작 2020.10) 기간이며 Java/Spring 실무는 없었음(GS네오텍 세션). 이 줄의 "Java/Spring 실무 있었음"은 무효. **Spring 서사는 비즈36.5(EXP-02) 중심으로 싣기로 결정.** 3년 요건은 "그에 준하는 역량" 트랙 + 면접 방어.
...
- 결정 (소유자 승인 2026-08-24, 스톱①):
  1. 연봉: **현재 기본연봉 5,500만원 / 총연봉(변동성과급 포함) 5,700만원 기재** (소유자 지정 2026-08-25로 갱신, 이전 승인은 둘 다 5,700) — 코오롱베니트 제출본 한정 예외 재승인(GS글로벌 전례와 동일 취지, 다른 문서 노출 금지 유지). 1p 경력 표 연봉란은 총연봉 기준 5,700 만원. 희망연봉은 양식 prefill "협의" 유지
  ...
  5. 매출 1.0억 지표(metric-2025-revenue-breakdown needs_scope): GS 한정 예외를 **코오롱베니트 제출본으로 확대 소유자 승인 (2026-08-24)** — 동일 스코프 병기 문면 한정, registry에 기록
- 경력사항 프로젝트 순서: 시간 역순(① AI 프로세스 ② 비즈36.5 ③ AI코치) 유지 — 양식이 "(최근 경력 순)"을 명시해 지정양식 강제가 하우스 F-04(JD 연관도 순)보다 우선 (reviewer R1 S-4 판단 기록)
- 작성 주체 예외 (2026-08-24): writer 서브에이전트 3회 연속 API 529 중단으로 초안 재조립을 최상위 에이전트가 직접 수행 — GS 확정본 문면 재사용 + companion 전 항목 라벨링, reviewer 전수 무결성 대조로 검증(재사용 구간 자모 일치 확인, must-fix 3건 처리)
- 하우스 규칙 예외 (지정양식 사유, 2026-08-25 기록): F-01 경력기술서 고정 골격·F-02 핵심 성과 섹션·F-03 헤드라인·F-07 섹션 순서·F-08 전면 계층 구성 미적용 — 써치라인 양식 구조([핵심역량]/[경력사항] 자유 단락)가 우선 (GS 전례와 동일 취지). **S-03 예외**: 미스미 "약 50% 개선(GA 기준)"·"32% 증가(GA 기준)"는 전후값 미확보 지표라 전후쌍 형식 충족 불가 — GS 소유자 확정 승계 + 2026-08-24 갭 인터뷰 GA 스코프 병기 결정으로 문면 유지
- 게이트 이력: R1 76/71 → R2 75.5/71 FAIL(캡 2라운드) → 소유자 지시 갭 인터뷰 4건(MyBatis·헬스체크 롤백·GA 스코프·장애 사례 없음) 보강 라운드(비산입) 71/74.5 → R3(최종 캡) 69.5/77 FAIL·정지 → 소유자 지시 추가 갭 3건(테이블 신규 추가형·BFF 설계/구현 주도·공통화 기준) + 3차 갭 3건(운영 문의·호환성 대응/BFF 타임아웃·오류 응답/PK·FK·조회 인덱스 설계) 보강 재채점(비산입) 72.5/73.9 FAIL — **전 라운드 블로커 0**. 소유자 "정직한 상향지원 출고" 결정 (2026-08-25). 잔여 감점은 구조적: 사내 메신저·웹하드 무경험(미주장), 경력 공백 미기재(소유자 결정), 장애급 인시던트 사례 없음, Java·DB 심화 세부 미확인
- 면접 대비 (채점관 기록): ① 운영 장애 질문 — "지표 대시보드·헬스체크 롤백까지가 범위, 온콜 장애 대응 경험은 없음"으로 경계 선답 ② Electron 실수정 범위(유지보수 스코프 밖 단정 금지) ③ 신규 테이블 정규화·인덱스 컬럼 구성(미확인 — 정직 답변) ④ BFF 타임아웃 값·부분 실패 정책(미확인) ⑤ 9주 단독 전환의 작업 단위·회귀 검증 ⑥ 매출 1.0억 귀속·체류 32%/로딩 50% 측정 구간 ⑦ 메신저 도메인 전이 설계 사고(SSE→양방향) ⑧ TOEIC 2026.05 만료 ⑨ Node BFF "신규 구축"은 설계·구현 주도+팀 리뷰 범위 ⑩ 근속: 이관 표기이나 실제 퇴사+신규입사(근속 리셋·경력증명서 2장)
- Draft status: **완료(정직한 상향지원 출고, 2026-08-25)** — 스톱② diff 승인 후 지정양식 주입·렌더 검증 완료. 제출본: `outputs/코오롱베니트_플랫폼개발운영_정다훈_써치라인.docx` (10p: 1p 코오롱베니트 입사지원서(w:sdt 콘텐츠 컨트롤 — python-docx tables/paragraphs가 건너뛰므로 lxml 직접 주입 필요, 사진·병역·학력·경력·자격·날짜·서명 포함) / 2~8p 써치라인 이력서·자소서 / 9~10p 개인정보 동의·서명 2026/08/25). 초안 정본 `outputs/kolon-benit-platform-resume-draft.md`, companion `outputs/kolon-benit-platform-new-prose.md`(K-1~K-37). 주입 스크립트: 세션 스크래치 inject_kolon.py (격자 삽입 전 표 참조 캡처·sdt 셀 채우기·NUMPAGES 정적 10 치환 주의). Codex 미사용(P-02)
- 렌더 개정 (2026-08-25, 소유자 grill-me "폰트·크기 불균일"): 타이포그래피 통일 — 주입 텍스트 전량 맑은 고딕 명시(1p sdt 셀은 테마 세리프 상속 방지 rFonts 필수), 크기 3계층(양식 원본 헤딩 13 불변 > 본문 9.5 > 보조 8.5: 기술 줄·격자 라벨·※ 각주 / 1p 셀은 9·8.5), 회사 양식 원본 요소(헤딩·타이틀·동의서·체크박스) 불변. 근무사항 ※ 각주 주입 누락 보완. **문면(md 정본) 불변 — 렌더 전용 변경, 재채점 불요** (GS v5 전례)
- 렌더 개정 2차 (2026-08-25, 소유자 "포맷이 읽기 불편" → portfolio-ux-reviewer 크리틱 14건 트리아지 수용 8·기각 4·자연해소 2): ① 고아 헤딩 해소 — 섹션 헤딩~첫 내용 keep_with_next 체인 ② 프로젝트 블록 페이지 잘림 방지 — 격자 행 cantSplit(긴 [주요 실행] 행만 분할 허용 — 전면 cantSplit 시 행 통째 밀림으로 반쪽 공백 페이지 발생, 1차 시도에서 확인) + [프로젝트]·[기간] 행 keep_with_next ③ 개인정보 동의서 앞 강제 페이지 브레이크(단독 페이지화) ④ 핵심역량 블록 간격 8pt·본문 들여쓰기 ⑤ 프로젝트 경계 여백 8pt ⑥ 자소서 문단 간격 6pt ⑦ "(최근 경력 순)" 검정 통일. 기각: 사진 축소(양식 틀 크기 준수)·성과/실행 마커 통일(의도된 구분)·입사가능시기 헤딩 격하(양식 동급 섹션)·열 좌표(이미 정렬). 총 10p 유지, NUMPAGES 정적 값은 최종 렌더 페이지 수와 일치 확인 필수. **문면 불변 — 재채점 불요**
- 렌더 개정 3차 (2026-08-25, 소유자 "여전히 오류" → GS v7 기준 대조): 근본 원인 = **양식 문서 기본값 차이** — GS 양식은 docDefaults 행간 1.08배(line=259 auto)+문단 뒤 8pt인데 써치라인 양식은 행간 1.0배+0pt라 동일 주입 수치로도 빽빽하게 렌더됨. 조치: ① 주입 문단 전량에 행간 1.08배(line=259 auto) 명시 이식(원본 양식 문단은 불변) ② 섹션 헤딩 앞 간격 14pt 복원(주입 시 원본 빈 문단 제거로 헤딩이 앞 내용과 밀착됐던 결함) ③ 총 11p로 증가, NUMPAGES 11 동기화. 교훈: **지정양식 주입 시 대상 양식의 docDefaults(행간·문단 간격)를 먼저 확인하고, 기준 렌더(GS v7)와 다르면 주입 문단에 행간을 명시 이식할 것**. 문면 불변 — 재채점 불요
- 가독성 재분해 (2026-08-25, 소유자 grill-me "전체적으로 읽기 힘듦"): 정량 진단 — 개괄 487자 4문장(GS의 2배)·핵심역량 최장 불릿 289자 등, 게이트 보강 라운드마다 절을 쉼표로 이어붙여 GS v6 "1줄 1메시지" 이전 상태로 회귀해 있었음. 조치: 핵심역량 5불릿 절 단위 재분해·개괄 2문단 분리·자소서 지원동기 문단 분리(K-38~40) + 본문 우측 들여쓰기 6mm·격자 내용 열 14.2cm(GS 실효 폭 — 페이지 여백은 GS도 동일 171.9mm였음이 실측 확인, 여백 확대 승인은 콘텐츠 들여쓰기로 구현해 1p 공식 양식 무영향). 무결성은 백업본 대비 토큰 diff로 기계 입증(변경 9건 전부 경계 구두점·어미, 사실 손실 0) — reviewer 재검 대체. 기존본은 _v1.docx·-v1-backup.md로 보존. 교훈: **갭 보강으로 문장에 절을 이어붙일 때마다 문장 길이가 자라므로, 출고 전 "1줄 1메시지" 재분해 패스를 기본 절차로 둘 것**

### 지오영 / AI 개발 (경력, 팀원) — 헤드헌터(㈜피플렙) 경유

- Source job posting: 헤드헌터(㈜피플렙, 임덕빈) 제공 JD 텍스트 + 피플렙 지정양식 이력서 docx·개인정보수집동의서 docx, 2026-09-09 수집
- Application deadline: [미확인 — 헤드헌터 경유, 2명 채용]
- Target position: AI 개발 (2명) — 신설부서(경영진 CFO 직할 검토 중) / 팀원 / 서울 서대문구 / 급여 AI 개발 시장 수준·면접 협의 / 전형 서류–기술인터뷰–최종
- Company (JD 기재분): ㈜지오영 (www.geo-young.com) — 2002.05.27 설립, 대표 조선혜 회장, 매출 4조 4,386억(2023) → '24년 연매출 5조 달성, 22년간 연평균 18% 성장, 본사 서울 서대문구 성산로 321, 직원 594명. 국내 최대 종합헬스케어 플랫폼: ① 의약품 유통(전국 약국 80% 거래처, 업계 1위) ② 헬스케어 IT(약국 점유율 55%·종합병원 90%) ③ 병원 구매대행 ④ 방사성 의약품 제조·유통(각 1위)
- 지원 조건: 1985~1992년생(만 35~42세) — 소유자 1991년생(만 35세) 충족 / 3~10년 실무개발 경력(개발 베이스 필수) — 5년 11개월 충족
- Job scope: ① 사내 AI 에이전트·애플리케이션 개발(LLM 기반 자체 에이전트·워크플로우 앱 직접 설계·구현) ② AX 실행(현업 비효율·요구 발굴 → AI 중심 솔루션 내재화) ③ 사내 혁신 리딩(Claude·OpenAI 등 최신 AI 모델·도구 선제 도입, 전사 생산성)
- Required skills: Python 기반 개발 + LLM API(Claude·OpenAI) 활용 애플리케이션 개발 유경험 / AI 에이전트(Agent) 아키텍처 구축 유경험 / 최신 AI 트렌드 빠른 흡수·즉시 적용 학습력
- Preferred skills: Multi-Agent 시스템 설계·MCP 활용 / Claude Code·Cursor 등 AI 코딩 도구로 높은 생산성 증명 / FastAPI 등 백엔드 프레임워크·AWS/Azure 클라우드 / 사내 업무 자동화·RPA 전환 주도 / 비개발 직군(현업)과 소통·문제 정의·협업 / 업무 관련 자격증
- Role preset: ax-harness (근거: GS글로벌 FDE·GS리테일 AX와 동일 계열 — AI 활용 내재화·현업 문제 정의·재사용 표준 신호. 단 이 JD는 "에이전트·워크플로우 앱 직접 설계·구현"이 담당업무 1번이라 EXP-01(LLM 앱 제품화)을 EXP-03과 동급으로 전진)
- Screen profile: **AI-Product / Pass bar 80 제안 — 소유자 미승인(2026-09-09 자율 세션)** → 게이트는 규칙대로 Balanced 80 + 경고로 채점
- 제출 형식: 피플렙 지정양식 docx(기본사항 표: 지원회사·성명(생년·만나이)·주소·병역·현재연봉·희망연봉·입사가능일·사진 / 학력 표 / 경력요약 표(년월수·이직사유) / 핵심역량 3~5개 자유 / 자격·교육·기타 표 / 상세경력(■ 회사별 [회사소개][주요업무][이직사유]) / 자기소개서 3항목(본인 소개 및 성격 특장점·직무수행 역량 및 자세·지원동기 및 입사 후 포부) / "지원자 拜上") + 개인정보수집동의서 docx(체크박스·날짜·성명·출생연도·이메일·전화). JD 명시: "수행한 AI프로젝트와 본인의 기여도 필수 기재" — 상세경력 1·2번이 AI 프로젝트, 기여도는 역할어(총괄·직접 제작·참여)로 표기. 파일명 규칙: `성명(만00세)_포지션명_회사명_(주)피플렙_임덕빈2026.docx`
- Candidate evidence to emphasize: EXP-01 AI 챗봇 제품화(LLM 앱, SSE, 오류 정책, Python 프롬프트·후처리 참여, RAG 참여) / EXP-03 하네스·MCP 서버 2종·Claude Code/Codex 연동·개발 표준 팀 정착·회귀 평가 도구(↔에이전트 도구 연동·AI 코딩 도구 생산성) / EXP-03 기획·운영팀 교육·GA4/PostHog 비개발 직군 지표 확인(↔비개발 직군 협업·AX) / EXP-02·EXP-07 풀스택·Node BFF·Docker·AWS(↔백엔드·클라우드) / 정보처리기사·AZ-900(↔자격증) / 헬스케어 도메인(대웅 계열 임직원 건강검진·AI 건강코치)
- Risks or gaps:
  - [구조적] "LLM 기반 자체 에이전트·워크플로우 앱 직접 설계·구현"·"AI 에이전트 아키텍처 구축" — 은행 근거는 MCP 서버 개발·도구 호출 환경 구성·하네스 수준(EXP-03 확대 해석 금지 가드). 에이전트 프레임워크·멀티 에이전트 오케스트레이션 설계 미주장. **갭 인터뷰 대상**(AI 오케스트레이션 토이 프로젝트 2026-07·AI 소프트웨어 사이드 프로젝트·이 my-recruit 하네스 자체 — 은행 미적재/[확인 필요])
  - [구조적] Python "참여·기여" 톤 한정(EXP-07 가드) — Python 단독 앱 개발 미주장. pandas 인용 금지(철회)
  - FastAPI: EXP-01 기술 스택에 [사용 위치 확인 필요] — GS 제출본 기술 줄 토큰으로만 재사용, 서술 금지
  - RPA 무경험 — 미주장, 개발 프로세스 자동화·배포 자동화로 인접 어필만
  - Multi-Agent: 2026-09-09 갱신 2 — 소유자 user-attested "RAG·VectorDB·멀티 에이전트·Redis 연결·SSE FE 표현까지 다 할 줄 안다(지식 보유, 실무 코드 소유는 FE)" + documentary 개인 프로토타입 3건. 능력 주장은 근거 병기형(프로토타입)으로만 문면화, 데모 저장소(my-recruit-8kl) 완성 후 근거 승격 예정. / 2026-09-09 갱신 1 — 은행 EXP-01에 "멀티 에이전트 설계·구현 참여"(Python, LLM 릴레이 서버·API 구축, 프롬프트·컨텍스트·하네스 담당, RAG·VectorDB는 LLM 개발자)로 적재돼 "참여"까지 주장 가능. 구조 형태·본인 구현 에이전트 범위는 여전히 [확인 필요] — 갭 인터뷰 대상 / Cursor 미사용(Claude Code·Codex만)
  - 다나아데이터 회사 개요 수치: 웹서치(사람인 2026) 매출 4억 5,314만원·직원 9명 — 팀 10명 이관 사실과 정합 불명이라 문면 미기재, 대웅제약 수치로 대체(GS·코오롱 전례) [소유자 확인 필요]
- Keywords to include: AI 에이전트, LLM, Claude, OpenAI, Python, MCP, Claude Code, Codex, 워크플로우, AX, 업무 자동화, 현업, 비개발 직군, FastAPI, AWS, Azure, RAG, VectorDB, 프롬프트, 하네스, 헬스케어
- 하우스 규칙 예외 (지정양식 사유, GS·코오롱 전례 동일): F-01 경력기술서 고정 골격·F-02 핵심 성과 섹션·F-03 헤드라인·F-07 섹션 순서·F-08 전면 계층 구성 미적용 — 피플렙 양식 구조([핵심역량] 3~5개 / ■ 회사별 상세경력 / 자소서 3항목)가 우선. 상세경력 프로젝트 순서는 양식이 시간순을 강제하지 않아 하우스 F-04(JD 연관도 순) 적용 — v2 개정 순서: AI코치 설계·구현 → AI 개발·검증 프로세스 AX → 건강관리실·검진센터 AX 기획 → 비즈36.5 레거시 현대화. C-08 연봉 미기재 예외: 양식 필수 항목이라 기본급 5,500만원(변동성과급 포함 총 5,700만원) / 희망 "면접 시 협의" 기재 — **소유자 재승인 필요(GS·코오롱은 제출본 한정 예외였음)**
- 하우스 규칙 예외 추가 (2026-09-09, 크리틱 R2 기록): **C-03 줄 첫머리 강조** — 자소서 문단 첫머리 볼드는 GS v7·코오롱 v4 제출본(소유자 확정 문면) 승계라 위반으로 되돌리지 않음. **F-10 회사 개요 3항목** — ■ 블록 하나가 두 법인(대웅제약→다나아데이터)을 담아 회사명 병기·이관 문장 포함이 필요, 예외. **C-04 제품명** — "AI 건강검진 챗봇(AI코치)" 병기 후 두 이름 혼용은 base-resume·GS·코오롱 전례 승계.
- 표기 정책 기록 (2026-09-09): 대웅제약→다나아데이터는 memory 결정대로 "계열사 조직 이관·담당 서비스·업무 연속" 표기 승계(경력요약 2행 분리 + 각주). 실제 퇴사(2026.05.31)+신규 입사(2026.06.01)·경력증명서 2장은 면접·평판조회 시 선답.
- 크리틱 이력 (2026-09-09): reviewer R1 must-fix 7(제품명·가능형·Agent 소제목·AWS 귀속·연봉 승인·companion orphan·줄 첫머리 볼드) / 적대 R1 27건(critical 5·major 9·minor 13) → 삼분 처리(수용: Agent PoC 소제목·LLM 앱 설계·구현 헤딩·AI 결론 비약·기술 줄 귀속·RPA 미끼 소제목·Config-Driven 중복·자소서 4 삭제·지원동기 상투구·회사 사실 완화 / 기각: Node.js 유지→R2에서 재지적돼 수용, 년월수 산정 기준·TOEIC 만료 표기·FP 발급기관·경력증명서 문구는 전례·소유자 판단 사항) → reviewer R2 must-fix 4(연봉 승인 대기·제품명 병기·Node.js 귀속·orphan) / 적대 R2 critical 3(10주↔2주 조기 기준선·오류 4유형 병합·연봉) → 수용: 오류 4유형 병렬 서술·Node.js 삭제·회사소개 2법인 분리·경력요약 각주·MCP "개발·수정" 통일·장애 절차 중복 제거·JD 필수③ 학습력 매핑(세미나 불릿)·핵심역량 ⑤ 불릿 분해·자소서 BFF 담당 범위 정밀화·지원동기 인과 병렬화 / 기각(전례·소유자 확정 문면): 10주↔2주 조기 병기, 자소서① 세미나 인과, 12개 도메인 모듈 예시, 3,493명 반복, 렌더링 비용 기준 표현.
- Draft status: **초안·크리틱 2라운드 완료(2026-09-09, 자율 세션) — outputs/geoyoung-ai-dev-{resume-draft.md,new-prose.md} + docx 2종 주입 완료** 
- 타겟팅 개정 (2026-09-09 grill-me, 소유자 지시): **포지셔닝 = "SW 이해도 있는 풀스택 개발자가 ML·딥러닝을 학습해 개발자 5명과 실제 AI SW를 설계·구현한 사람"**. FE 서술 최소화, JD에 충실. AI코치는 설계·구현/API 설계/Multi Agent 설계·구현 참여(Python), LLM 릴레이 서버·API 구축, 프롬프트·컨텍스트·하네스 엔지니어링 구축 — RAG·VectorDB 구현은 LLM 개발자 담당(같은 문장에 병기). AX는 FE·트러블슈팅·기획 AX(개발 영역 중심)를 한계로 인정하되, 비즈36.5 중심 건강관리실·검진센터 AX 전환(동의서 시스템·검진 진행현황 기획서 — 요구사항 분석·기능 설계·부서 협업, 기획 확정·상세 기획 중)을 현업 요구 발굴 근거로 세움. 자소서는 AI 인사이트("암묵지 → DX 기능화가 AX의 선행조건", 개발자는 판단만 하는 Figma→Agent→Storybook→Test 파이프라인) 중심으로 재작성.
- 수치 승인 (2026-09-09, 소유자 "다 올리고 승인"): 월별피드백 자가 기록 수치 10건을 metric-registry에 source_stated로 등재하고 지오영 제출본에 사용 승인 — Figma MCP 1주→1일, 차트 1시간→10분, React 전환 6주→2주(70여 화면), RESTful 29건 4주→1주, 자동 테스트 58→2,318건, 초기 로딩 3.53MB→0.35MB, PostHog 검수 1시간→20분(109건), E2E 표준 문서 4건, 비즈36.5 Storybook 93/258·버그 64건, 바이오에이지 Storybook 148·접근성 198건.
- 결정 (2026-09-09 grill-me): ① KT AICE Associate·빅데이터분석기사 필기 합격은 자격 표에 올리지 않고 자소서 학습 서사에만 녹임(소유자 지시). ② 내담씨앤씨 블록은 프로젝트 제목 없이 풀스택·성능·협업 실적 불릿 4개로 압축(선택지 ⓐ). ③ 상세경력 무게는 대웅·다나아 블록(멀티 에이전트 AI SW·건강관리실/검진센터 AX 기획·AI 개발 프로세스 AX)에 집중.
- 게이트 이력: R1(Balanced 80, 프리셋 미승인 경고) **인사 75.5 / 기술 66 — 둘 다 FAIL, 블로커 0**. 감점 핵심은 구조적 갭 — JD 1순위(에이전트·워크플로우 앱 직접 설계·구현, Python·LLM API 앱 개발)의 과거형 근거가 "MCP 서버 개발·수정 + 도구 호출 환경 구성 + Python 후처리 참여"에 그침(은행 확대 해석 금지 가드 준수). 기술담당자 권고 레버리지: ① MCP·에이전트 블록을 문제→설계 판단→결과로(도구 스키마·전송 방식·실패 처리·연동 효과 — 은행 미적재, 갭 인터뷰 대상) ② Python·LLM API 직접 구현 과거형 근거(AI 오케스트레이션 토이 프로젝트 2026-07·AI 소프트웨어 사이드 프로젝트·my-recruit 하네스 — 은행 [확인 필요]) ③ 9주 단독·2초대·100%의 방법 설명. R1 후 무근거 보강 없이 경량 수정만 반영(성과줄 순서·9주 전환 기제 병기) — 재채점 없이 정지, 소유자 갭 인터뷰 결정 대기(P-01 3라운드 캡 중 1라운드 소진).
- 코드 근거 조사·결정 (2026-09-09 grill-me): 소유자 지시로 `~/Documents/workspaces/my-health-ai-coach-llm-version2`(AI코치 v2 MAS 서버, 2026.04~08) 조사 — 서브에이전트 18개·8카테고리, 오케스트레이터 7단계, 라우팅 4경로, 3사 7모델 어댑터, Redis·Mongo·Qdrant 메모리, FAISS+Kiwi 하이브리드 RAG, PII/BOLA 가드레일, eval 240건 A/B 하네스. **정다훈 커밋 0건**(kimhyungjun 195/208) 확인·보고 → 소유자 "커밋 신경쓰지 말고 뽑아먹자" 지시로 아키텍처 사실을 "참여" 톤으로 사용(은행 기여 범위 가드·면접 방어 메모 적재). 본인 커밋 산출물도 반영: 웹 저장소 GPT-4o 에러 분석 파이프라인(scripts/error-report, CI 잡 정의·러너 제약으로 미가동)·LLM 회귀 검수 도구·SSE 목 서버·Claude Code 하네스(에이전트 7·스킬 8), 개인 프로토타입 chatbot-boilerplate(LLM 어댑터 4종·SSE·프록시, 2026.01)·mcp_test(FastMCP stdio 서버+LangGraph 클라이언트, 2025.10)·test-vector(Qdrant RAG, 2025.07). v3 문면: 상세경력에 "AI코치 v2 멀티 에이전트 서버 전환 참여(2026.04~08)" 항목 신설(시기는 저장소 커밋 범위 추정 — 소유자 확인 필요), 핵심역량 ②에 GPT-4o 파이프라인, 자소서에 프로토타입 학습 서사. 보안 메모: test-vector/vector_app.py Google API 키 평문 — 소유자에게 폐기·교체 권고.
- 하우스 규칙 예외 추가 (2026-09-09, reviewer v3): **S-03 예외 승계** — 미스미 "평균 로딩 약 50% 개선(GA 기준)"은 전후값 미확보 지표라 전후쌍 불가(GS·코오롱 전례 동일).
- v3 크리틱 (2026-09-09): reviewer v3 must-fix 5(연봉 승인·v2 참여 기간 무근거·BOLA 세부·참여 톤·S-03 예외) / 적대 v3 critical 3(v1·v2 시기 융합, 상세경력 2 본인 구현으로 읽힘, 본인 담당 불릿 부재)·major 11 → 수용: v1/v2 시점 분리(핵심역량 ①·자소서 1), 상세경력 2를 "시스템 구성 :"+"본인 담당 :"+"구성" 라벨로 재작성(볼드 제거·TTFT 주장 제거·BOLA는 은행 문면), 기간 "(2026, v1 이후)"로 완화, 기술 줄 협업 스택 분리, 7단계 정합, 프로토타입 3건 핵심역량 ② 승격(개인 한정어), GPT-4o 파이프라인 언어·실행 형태 병기, 상세경력 3·5 압축(React 전환 확장·GA4 불릿 제거, API 계층 통합), 미스미 한국 웹 스코프, 개발자 5명 반복 축소 / 기각(전례·소유자 확정): 10주↔2주 조기 병기, 기획자와 MVP 범위, 년월수 산정 기준. 소유자 확인 대기: v2 참여 실제 시기·형태, 릴레이 서버 시기(v1/v2), FP·JLPT 발급기관·N1 표기.
- 게이트 이력 R2 (2026-09-09, v2 재포지셔닝판 — reviewer v2 must-fix 7·적대 v2 critical 3/major 9 삼분 처리 후): **인사 75 / 기술 65.5 — 둘 다 FAIL, 블로커 0**. 두 채점관 공통 지적: JD 1순위(에이전트 아키텍처·LLM 앱 직접 구현)의 근거가 "멀티 에이전트 설계·구현 참여, 릴레이 서버·API 구축, MCP 서버 개발·수정" 한 문장 수준으로 구현 층위(에이전트 구성·라우팅·툴 스키마·전송·실패 처리·릴레이 서버 동시성/재시도/폴백)가 0 — 은행 [확인 필요] 항목이라 문면으로 해소 불가, 갭 인터뷰 필요. 그 외: 10주↔2주 조기 기준선(4개 리뷰 반복 지적), 100%(렌더링 지표)를 AI 품질 결론 자리에 배치, 58→2,318건 생성 방식 미기재, 14개월 내 소속 2회 변경 스캔 신호. 기술 인터뷰 필수 질문 5개 기록됨(MCP 코드 수준·멀티 에이전트 화이트보드·릴레이 서버 설계 결정·수치 기준선·0→1 직접 구현 여부).
- 프레이밍 결정 (2026-09-09 grill-me, 소유자): "다 했다"는 쓰지 않되(소유자 요청을 no-fabrication 근거로 거절, 소유자 수용) **"실제 연구에 참여·기여했다"는 프레이밍으로 폼나게** — v4: 핵심역량 ①에 능력 주장("…설계·구현할 수 있는 전 구간 역량 — 근거 : 개인 프로토타입 3건·v1/v2 참여·AICE Associate·빅분기 필기"), 상세경력 2 "연구·개발 참여·기여"(기여 범위 명시, "코드는 LLM 개발자 주도"류 축소 한정어는 C-01에 따라 문면 제거·은행 유지), 자소서 1 전 구간 서사. **데모 저장소 구축 의향 확인** — 스펙 `outputs/ai-agent-demo-spec.md`, beads my-recruit-8kl. 완성 시 이력서 근거로 추가.
- 보강 라운드 (2026-09-09, v4 참여·기여 프레이밍, 캡 비산입): **인사 59(블로커 1 캡 — 핵심역량 ① 능력 주장 "Redis 세션·멀티 에이전트 설계·구현 가능"이 Risks 미주장 경계 초과 + AICE·빅분기 핵심역량 재노출은 소유자 결정 위반) / 기술 63 — FAIL.** reviewer v4 must-fix 5(Redis 무근거·AICE 재노출·SSE 프론트 연동 v2 귀속·자소서 참여 형태 단정+v1/v2 융합·연봉) 전건 반영: 능력 불릿을 근거형("개인 프로토타입으로 … 직접 구현 — 전 구간을 다룰 수 있는 근거")으로 교체, AICE·빅분기는 자소서에만, v2 기여 범위를 은행 확정 3건(설계 논의·프롬프트/컨텍스트·검증 하네스)으로 한정, 자소서 마지막 문장 확정 사실만으로 재작성, 시스템 규모 줄 "→"→"시스템 :" 라벨, 메모리·RAG 불릿 시스템 사실화, 성과줄 "설계·구현에 참여" 통일. 재채점 없음. **4라운드 공통 결론: 문면 기술로는 80 불가 — 본인 명의 구현 산출물(데모 저장소) 또는 MCP 서버·릴레이 서버 구현 세부(갭 인터뷰)가 유일한 상승 경로.**
- 게이트 이력 R3 (2026-09-09, v3 — MAS 아키텍처·프로토타입 반영판): **인사 68 / 기술 64.5 — 둘 다 FAIL, 블로커 0. R2보다 하락.** 원인: 상세경력 2(v2 MAS)가 "시스템 구성" 카탈로그 + "코드는 LLM 개발자 주도" 라벨이라 기술담당자가 블록 전체를 할인, 본인 담당 3건(설계 논의·프롬프트·하네스)에 구현 층위 0, 전문용어 밀도로 인사 6초 스캔 하락, 핵심역량 전후쌍 7개 적층. 기술담당자 권고: ① 상세경력 2를 본인 담당 3건의 문제→판단→결과로 뒤집기(은행 미적재 — 갭 인터뷰 선행) ② 사내 MCP 서버 2종·릴레이 서버에 도구 스키마·전송/타임아웃·실패 처리 3요소(체감 +6~8) ③ 수치 절반 감량+측정 기준. 게이트 후 무근거 보강 없이 압축만 적용(구성 6불릿→2줄, 핵심역량 수치 3개 제거) — 재채점 없음(캡 소진).
- 정돈 개정 v5 (2026-09-09, 소유자 "있는 거 없는 거 다 때려넣은 이력서 같다 / 코오롱베니트 완료본 레퍼런스 / 문단 양식"): **문면 F-10 재분해** — 진단: 핵심역량 불릿 115~259자·실행 불릿 최대 239자(코오롱 v4는 각 35~75자·85~140자)로 2~4배 초과. 조치: 핵심역량 소제목을 짧은 명사구로 분리하고 불릿 15개를 전부 1줄(≤72자)로, 실행 불릿 ≤92자로 재분해. 회사소개를 F-10 3항목 1줄(업종·매출·인원)로 바꾸고 이관 설명은 ※ 각주 분리. 프로젝트 제목/기간 분리(제목 줄 + 기간 메타 줄). 내담 블록 1프로젝트 압축. **문단 서식 전면 재작성**: 템플릿 pPr 상속 대신 명시 지정(행간 1.08배 line=259 auto, 들여쓰기 계층 200/420/460 + hanging, 성과·실행 그룹 간 90 간격, 기술 줄 8.5pt 회색, 회사소개·기간 메타 회색), 한글↔숫자 자동 자간 해제(no_autospace — "만 35 세"→"만35세", "10 주"→"10주"; 템플릿에 이미 켜진 autoSpaceDE/DN도 val=0으로 덮어씀), 핵심역량 ○ 그룹·섹션 헤딩 keepNext 체인(고아 방지, 코오롱 v4 전례), 자소서 양쪽정렬→왼쪽정렬(한글 어절 벌어짐 해소). **렌더 검증 완료: Word 변환 8페이지, 전 페이지 육안 확인** (v1 대비 문면 축약·페이지 균형 개선). 문면 축약이므로 사실 추가 없음 — reviewer 무결성 재검 실행.
- v5 무결성 재검 (2026-09-09, reviewer): must-fix 5 / should-fix 10. **축약이 유발한 귀속·스코프 소실 7건** — 코오롱 세션(압축→귀속 소실 7건) 동일 패턴 재현. 반영: 대웅 블록 근속 1년03→1년02개월(표·총계 정합), 핵심역량 10주→2주에 "(화면·테마 기준)" 병기(C-05), Figma MCP 귀속·비즈36.5 서비스명 복원, 에러 파이프라인 "TypeScript, CI 잡 정의·로컬 실행" 병기, "RAG·VectorDB 연동 구조 협의"(은행 무근거)→"구현은 LLM 개발자 담당", 시각 회귀 "임계값 2%", v2 MAS 성과줄 "→"→"시스템 :"·본인 담당 불릿 볼드 분리·기술 줄 협업 스택 분리(기여 범위 가드), 자소서 Figma 체인 완료형→추진형·"Design.md"(무근거 토큰) 삭제, 이직사유 AX 스코프 앵커링(C-09), 자소서 첫 문장 "풀스택으로 시작" 뉘앙스 정정, 반복 수치 1회 축소. **기각(전례·소유자 확정)**: 기술 문서 48건(daewoong-history 충돌 미해소이나 GS v7·코오롱 v4 제출본 계속 사용), AX 단계 "상세 기획 진행 중"(소유자 2026-09-09 발언 "기획서를 상세하게 구체화하고 있고"가 은행 2026-09-08 기록보다 최신), 핵심역량 Node.js API 설계 주도(EXP-07 정밀화 근거 있음, R2 삭제분은 AI코치 기술 줄 건이라 별개). **잔존 must-fix 1건: C-08 연봉 예외 소유자 재승인** — 출고 전 필수.
- v14 docx 출고 (2026-09-10, 소유자 "그냥 이대로 md 파일을 중심으로 docx 만들어줘") — **최종 6페이지**(상세경력 3~4p, 자기소개서 5~6p, 빈 페이지 0). 레이아웃: 자기소개서 앞 페이지 나눔이 양식 고정이라 내담 블록이 4p 안에서 끝나야 5p 공백이 사라지는 구조 → 상세경력 본문 줄간격을 259(1.08배) → **240(1.0배)** 로 낮추고 실행 불릿 after 20→6, 제목 before 130→80, 회사 헤더 before 260→200 등으로 5회 반복 조정. `PPR()`에 `line` 인자 신설.: 검토용 md 확정 신호로 보고 `_v14` 접미사로 docx 2종 생성(기존 `_v9` 등 이전 파일 보존). 문면 대조 — 핵심역량 14건·상세경력 22건·자소서 15문단·이직사유 2건·회사소개/각주 3건 전부 미반영 0건, XML 파싱·zip 무결성 OK. **엔트리에 기록된 신규 주장 3건(기능 4건 적용 / RAG·FastMCP 실무 적용 / 차기 버전 개발 수행)과 문장 오류 1건은 소유자 지시로 수정 없이 그대로 출고** — 은행 갱신 또는 면접 방어 준비 필요.
- **v14 소유자 전면 교체본 (2026-09-10, 소유자가 완성 문면 전체를 붙여넣어 "다음 내용으로 변경해줘")**: 경력요약·핵심역량·상세경력·자기소개서를 소유자 문면으로 통째 교체하고 원문 대조로 일치 확인(13개 표본 불일치 0건). 계측: 핵심역량 5그룹 15불릿 → **4그룹 14불릿**(AI 서비스 개발 / AI 개발 도구 연동·테스트 자동화 / 현업 요구사항 분석·서비스 기획 / 풀스택 개발·배포 개선), 상세경력 19 → 22줄(성과 선행 '→' 줄 폐지, 실행 불릿 단일 계층), 자소서 2,547 → **2,484자**·15문단. 소유자 문면이라 어투·구성은 그대로 유지한다.
  - **신규 주장 3건 — 은행 근거 초과, 소유자 결정으로 반영하되 확인 필요**: ① `실사용 플랫폼에 기능 4건 적용` / `기능 4건을 건강검진 플랫폼에 적용하여 경쟁력을 확보` — 은행 "비즈36.5 2단계 건강관리실·검진센터 AX 기획" 항목은 Result·Metrics가 `[확인 필요 — 기획 확정 단계이며 배포·운영 성과 없음. 성과 서술 금지]`로 적재돼 있어 배포·적용 주장은 근거 밖. **은행 갱신 또는 문면 완화 필요** ② `AI챗봇을 위해 Qdrant 기반 RAG와 FastMCP 에이전트 서버를 구현 … 실무에 적용하여` 및 핵심역량 `Qdrant 기반 RAG, FastMCP 에이전트 서버, LLM 어댑터를 이용한 SSE 챗봇 구현` — 기존 문면·은행은 이를 **업무 외 개인 프로토타입**으로 한정했고 AI코치의 RAG·VectorDB는 LLM 개발자 담당이 가드. 실무 구현으로 읽히면 상세경력 P1의 "RAG·VectorDB 구현은 LLM 개발자가 담당"과 충돌 ③ `차기 버전 연구에서도 에이전트 실행·연결 방식의 설계 논의 및 개발을 수행` — 은행 v2 MAS 항목은 소유자 커밋 0건 가드가 있어 "설계 논의 참여"까지가 한계, "개발 수행"은 초과. 또 `AI와 관련된 다양한 컨퍼런스를 참여 및 논문을 확인` 중 논문 확인은 은행 미적재(컨퍼런스 인사이트만 있음).
  - **문장 오류 1건(미수정, 소유자 확인 대기)**: "Qdrant 기반 RAG와 FastMCP 에이전트 서버를 **구현을 위해** 다양한 라이브러리 및 기술 및 논문들을 살펴보고" — 조사 중복('를 …을 위해')과 '및' 3회 중첩. 소유자 원문이라 임의 수정하지 않음.
- v13 성격 특장점 서사 확정 (2026-09-10, 소유자 지시 "'제 성격 특장점은 문제 정의가 면밀하다는 점입니다'로 바꾸고, SW를 만들고 AX로 전환하기 전에 면밀하게 문제를 파악하고 사용자의 행동양식을 많은 커뮤니케이션을 통해 누락 없을 수 있게 하고, 완료 이후에도 테스트 및 검증에 신경 쓴다"): 서두를 "문제 정의가 면밀하다"로 확정하고 본문을 소유자가 지정한 3단(전 단계 문제 파악 → 현업 커뮤니케이션으로 사용자 행동양식 누락 없이 정리 → 완료 후 테스트·검증) 으로 재구성. 근거는 은행 확정분만 사용 — 회귀평가 도구 직접 제작·E2E 배포 게이트 편입·반복 QA 3시간→30분. 단점 서사는 v12대로 미기재 유지. 계측: 자소서 2,547자, 항목1 문단 179/296/280자, 최장 300자, 볼드 45%.
- v12 성격 특장점 단점 삭제 (2026-09-10, 소유자 "본인 소개 및 성격 특장점에 굳이 단점을 적을 필요는 없을 것 같다"): 강점(문제 정의의 꼼꼼함) 1개만 남기고 약점·보완 서사를 제거. 회귀평가 도구·반복 QA 3시간→30분은 강점의 실행 근거로 재배치("정해 둔 판정 기준을 사람이 매번 확인하지 않도록 도구로 옮겨"). **하우스 룰 정합**: GS글로벌 전례의 "자소서 [성격의 장/단점]은 단점 시 보완 행동 사실 필수"는 장단점을 함께 묻는 항목에 적용되는 규칙이고, 피플렙 양식의 항목명은 `[본인 소개 및 성격 특장점]`(장점만)이므로 단점 미기재가 양식 항목명과 정합. 단점 어휘(약점·단점·부족·반면) 문면 0건 확인. 계측: 자소서 2,556 → 2,518자, 항목1 문단 179/296/251자, 최장 300자.
- v11 역량 3항목 근거 교체·성격 프레이밍 변경 (2026-09-10, 소유자 지시 "하고 있는 것 말고 해냈던 것. AX는 FE AX 전환으로 / 신기술 학습·도입은 실제 실무에서 도입했던 사항으로 / 커뮤니케이션은 기획·운영·건강관리실과 긴밀히 소통해 요구사항 정립해 기능으로 만들어낸 것으로" + "성격 특장점은 문제 정의에 대한 꼼꼼함으로"): ① **AX 전환 역량을 진행 중 기획 → 완료된 FE AX 전환으로 교체.** 근거는 2026-09-09 승인된 월별피드백 수치 — Figma MCP 리드타임 1주→1일, SKILL.md 표준화로 신규 차트 1시간→10분, 자동 테스트 58→2,318건, Storybook 93 스토리·258 UI 상태·버그 64건. 마무리는 소유자 인사이트("개발자는 판단만 하는 영역으로") 승계 ② **신기술 도입은 실제 도입 이력으로 교체** — Claude Code·Codex 실무 도입, MCP 서버 2종 직접 개발·연동, AI 명세 분석·구현·검증으로 RESTful API 29건 4주→1주, PostHog 109건 사이클 E2E 자동화로 전수검사 1시간→20분 ③ **커뮤니케이션을 건강관리실·기획·운영 협업으로 교체** — 동의서 업무 요구사항 정립(법무 확인 6건·제외 범위 6건 확정, 발송·회수·대리 등록·철회·권한 분리), 검진 진행현황은 현장 보건관리자 피드백으로 수검 판정 기준 확정·기존 회원관리 화면 재사용. **은행 가드 준수: 이 과제는 "기획 확정·배포 성과 없음"이므로 소제목·본문 모두 "기능으로 설계"까지만 쓰고 구현·운영 성과는 주장하지 않음**(소유자 표현 "기능으로 만들어낸"을 그대로 쓰지 않은 이유) ④ 성격 특장점을 "확인되지 않은 것을 그대로 넘기지 못한다" → **"문제를 정의하는 데 꼼꼼하다"**로 프레이밍 변경, 보완 행동은 기존 시그니처(회귀평가 도구·반복 QA 3시간→30분) 유지하되 "정해 둔 판정 기준을 사람이 매번 확인하지 않도록"으로 문제정의 축과 연결. 계측: 자소서 2,556자, 최장 문단 300자, 볼드 44%. 산출은 검토용 md만.
- v10 성격·역량 문면 개정 (2026-09-10, 소유자 지시 "성격 특장점은 이전 자기소개서를 찾아보고 한 가지만 드러나게 / 직무수행 역량도 이전 자기소개서 참고해 주제는 유지하되 내 뉘앙스가 드러나게 리라이팅"): Explore로 자기소개서 전 문면(md 초안·docx 제출본·reference 정본) 전수 조사. **소유자가 '성격' 라벨을 붙인 자리에는 예외 없이 단 하나의 성향만 등장**함을 확인 — "결과물을 충분히 검증하려는 성향 / 확인되지 않은 것을 그대로 넘기지 못하는 성향"(23개 문서, 지오영·GS글로벌·코오롱 3타깃) + 그 꼼꼼함을 개인 습관이 아니라 팀 프로세스·도구로 이전하는 보완 행동. 2순위 "팀이 반복해서 사용할 수 있는 기준으로 만드는 것이 강점"은 독립 성향이 아니라 같은 성향의 조직화 단계로 항상 짝 등장(18개 문서). 조치: ① 성격 문단을 **2문단 → 1문단**으로 합쳐 성향 1개만 노출(강점 → 약점 → 보완 행동 3시간→30분 → 결론). 중복 근거(400건 발화 100%)는 항목1 P2·상세경력에 있으므로 제거 ② 직무수행 역량 3항목은 소제목 주제(AX 전환 역량·신기술 학습 및 도입 역량·커뮤니케이션) 유지하고 본문을 소유자 기존 시그니처 문장으로 리라이팅 — "요청을 바로 구현하기보다 업무와 데이터 구조를 먼저 파악하는 것이 제 문제 해결 방식입니다"(11개 문서), "새로운 기술을 개인의 노하우로 끝내지 않고 팀이 반복해서 사용할 수 있는 기준으로 만드는 것이 강점입니다"(18개 문서), "원리를 먼저 확인하고 실무에서 효과와 한계를 검증한 뒤"(self-intro v2), "역할과 직군이 달라도 근거와 영향 범위를 기준으로 논의"(NHN Dooray) + "서로 다른 요구를 반복 가능한 제품 규칙으로"(네이버웹툰), "하나의 서비스에서 여러 상품을 운영하기 위한 제품 설계 문제로 정의"(base 정본) 승계. 계측: 자소서 2,405 → **2,505자**, 최장 문단 297자, 볼드 45%, 볼드 없는 문단 0개. 산출은 검토용 md만(docx 미빌드).
- **[확인 필요] 반복 QA 지표 문서 간 불일치**: 성격 문단 핵심 수치가 지오영·GS글로벌·코오롱 제출본은 **3시간 → 30분**, 넥스트증권·카카오페이증권·NHN Dooray 제출본은 **3시간 → 1시간**이다(원본 평가문서는 1시간). metric-registry `metric-qa-time` 기준으로 30분을 쓰고 있으나 제출 전 소유자 확정 필요.
- 산출 방식 전환 (2026-09-10, 소유자 지시 "우선 md 파일로 써봐. docx는 이제 내가 확정할 때까지 계속 md로 쓸 거야"): 이후 개정은 **`outputs/geoyoung-ai-dev-review.md`(검토용 md)만 갱신**하고 docx는 소유자 확정 신호가 있을 때만 빌드한다. 검토용 md는 파이프라인 스캐폴딩(출처 태그·파티션 주석·사진 경로) 없이 제출 문면 그대로 읽히게 `render_geoyoung_md.py`가 함께 생성한다(초안 md·companion은 무결성 검사용으로 계속 유지). 작성일·동의서 날짜는 2026.09.09 → **2026.09.10**으로 갱신(제출일 확정 시 재조정 필요). 현재 docx 최신본은 `_v9`(7페이지)이며 md 개정분은 아직 docx에 미반영.
- v9 자기소개서 개정 (2026-09-10, 소유자 피드백 "성격 특장점이 잘 드러나지 않는다 / 직무수행 역량은 보편적 언어를 소제목으로 쓰고 그에 맞춰 내용을 구체화(예: 커뮤니케이션·AX 전환 역량·신기술 학습 및 도입 역량) / 입사 후 포부는 JD를 더 확인해 사람 고유의 암묵지를 AX로 전환해 생산성 향상 + 헬스케어 도메인 역량으로 그것을 이룰 수 있다는 방향"): ① **항목1에 성격 특장점 2문단 신설** — v8은 프로젝트 서사만 있어 성향이 드러나지 않았음. 자기인식형 구성("확인되지 않은 것을 넘기지 못하는 성향" → AI 제품에서 강점으로 작동 → 같은 성향의 약점(확인 시간) → 개인 습관이 아니라 프로세스로 이전해 반복 QA 3시간→30분) ② **항목2 소제목을 보편 역량 언어로 교체** — 'AX 전환 역량 / 신기술 학습 및 도입 역량 / 커뮤니케이션'. JD 담당업무 ②AX 실행·③최신 모델 선제 도입과 우대 '비개발 직군 소통·문제 정의·협업'에 1:1 대응. 소제목당 1문단으로 압축 ③ **항목3 재작성** — 지원동기를 "암묵지를 기능과 AI로 옮겨 생산성을 올리는 일을 헬스케어에서" 로 세우고, JD 회사 사실(전국 약국 80% 거래처·약국·병원 헬스케어 IT·병원 구매대행)과 신설 부서 맥락을 근거로 "사업이 넓을수록 판단 기준이 담당자 경험에 쌓이고 그 암묵지를 기능으로 옮기는 것이 AX의 출발점"이라는 논지 → 실행 가능 근거로 헬스케어 도메인 경험(건강검진 플랫폼·AI 건강검진 챗봇 개발·운영, 건강관리실·검진센터 AX 기획) 명시. 계측: 자소서 1,811자 → **2,405자**(코오롱 1,909자 대비 +26%, v7 3,448자 대비 -30%), 최장 문단 296자, 볼드 36%, 볼드 없는 문단 0개. 항목 간 수치 중복 0건(반복 QA는 항목1에만, MCP·개발 표준은 항목2에만 배치). 출력은 `_v9` 새 파일.
- v8 정돈 개정 (2026-09-10, 소유자 피드백 "대웅 프로젝트가 너무 길고 '선반영 :' 같은 라벨이 마음에 안 든다 / 자기소개서도 너무 길고 중요한 부분에 볼드가 없다 / 코오롱베니트 참고"): 코오롱 v4 규격을 계측해 대조 후 3건 조치. ① **실행 불릿의 "라벨 : " 프리픽스 21개 전면 제거** — v7이 쓴 과제·판단·전제 선합의·본인 담당·병목 특정 등 메타 라벨을 기각하고, 소유자가 만족한 핵심역량 불릿과 동일한 평서형 한 줄 + 볼드 1개로 통일 ② **대웅 블록 24줄 → 15줄**(프로젝트당 성과 1~2 + 실행 2~3), 내담 6줄 → 4줄, 상세경력 총 30줄 → 19줄 ③ **자기소개서 3,448자 → 1,811자**(코오롱 1,909자와 동급), 문단 최장 352자 → 238자, 볼드 21% → **39%**, 볼드 없는 문단 3개 → **0개**. 사실은 전량 유지하고 문장만 재구성 — 기여 경계(v1 참여·담당 / v2 참여 / RAG·VectorDB는 LLM 개발자 담당)와 스코프 한정어(400건 발화 기준·화면·테마 기준·GA 기준·대용량·수만 건·소속 내담 SI·"검증·배포 가능한 체계") 전수 보존. 출력은 소유자 지시로 **덮어쓰지 않고 새 파일**(`_v8` 접미사)로 분리, 빌더에 argv[3] 출력 접미사 인자 추가.
- v6 무결성 재검 반영 (2026-09-09 reviewer): 다이어트가 유발한 손실 2건 + 가드 이탈 6건을 줄 수 추가 없이 복원. ① 내담 성능 개선의 **기제·귀속 분리 복원**(목록 검색 5초→1초 = FlatList 가상화·재렌더링 제거 / 대시보드 2.5초→1초대 = DB 인덱싱·응답 필드 축소) — 축약본은 "렌더링과 DB 양쪽"으로 뭉쳐 EXP-04의 "반드시 병기" 가드를 어겼음 ② AI코치 v1+v2 병합 블록 기간을 `2025.07~2025.10 제품화(개발자 5명) · 2026 v2 전환 참여`로 **시기·팀 규모 귀속 분리**(v1 근거는 2025.07~10, v2 참여 시기는 [확인 필요] 상태이므로 "2025.07~2026" 연속 관여 함축 제거) ③ MCP를 "직접 만들어" → "**사내 문서 검색·테스트 도구 호출용** MCP 서버와 검증 도구를 직접 **개발·수정**"(용도 한정어 + 적대 R2 수용 표현 통일 복원, v2 저장소 근거 오인 차단) ④ "형식과 **품질**이 매번 다른" → "형식이 매번 다른"(품질 편차는 은행 근거 밖) ⑤ P1 판단 줄에 본인 담당(프롬프트·컨텍스트·검증 하네스) 접합 — 역할 줄 삭제로 v1 구간 기여 경계가 상세경력에서 소거됐던 문제 ⑥ P3 판단 줄에 "부서 협업으로 요구사항 분석·기능 설계 수행" 접합(JD 담당업무 ② 유일 과거형 근거 블록의 실행 내용 복원) ⑦ 이직사유에 AX 스코프 앵커(C-09) 복원 ⑧ 내담 역할 줄에 아키텍처 귀속("일본 본사 설계 아키텍처 위에서 … 6인 팀 리드") 복원. 서식: 회사 블록 사이 빈 문단 제거 후 다음 헤더 before 260으로 대체 → **7페이지 유지**(내담 이직사유가 4쪽에 안착, 빈 페이지 0). 미해소: 연봉 기재 C-08 예외 소유자 재승인(코오롱 승인은 "제출본 한정"이라 재사용 불가), 핵심역량 2건(GPT-4o 파이프라인 실행 형태 병기 / 1불릿 2주장)은 **핵심역량 불변 지시**에 따라 소유자 판단 대기.
- 상세경력 다이어트 v6 (2026-09-09 grill-me, 소유자 "핵심역량과 중복이 많으니 상세경력을 다이어트"): 정량 측정 — 핵심역량 15불릿 중 **8개가 상세경력과 45%+ 중복**, 수치·고유명사 31개 동시 등장. 코오롱 v4 대비 분량은 오히려 적었음(41줄/3,091자 vs 60줄/3,694자)이라 문제는 총량이 아니라 중복·블록 수·시각적 평탄함으로 진단. 조치: ① 프로젝트 6개 → **5개**(AI코치 v1+v2 병합 — 반복 지적된 v1/v2 혼동도 동시 해소) ② 구조를 **역할·과제·판단·성과 라벨형**으로 전환 — 소유자 요구 "정량화보다 이 사람이 어떤 일을 했는지 인사이트"에 맞춰 수치를 프로젝트당 1~2개로 줄이고 판단 줄을 신설(채점관 지적 "전후쌍 12개 적층이 신뢰도를 서로 희석"·"프로세스 지표가 회사 이득을 흐림"과 정합) ③ 핵심역량과 100% 중복이던 **역할 줄 4개 삭제**(핵심역량은 소유자 지시로 불변, 내담은 핵심역량에 없어 역할 유지) ④ 기여 경계는 P1 '이후' 줄과 핵심역량 "참여"로 보존. 결과: **47줄 → 28줄(40% 감량), 중복 8건 → 1건, 8페이지 → 7페이지(빈 페이지 없음)**. 서식: 라벨 볼드 + hanging indent 990/530, 두 번째 회사 블록 앞 여백 축소. 문면 축약이므로 재채점 없이 무결성 재검.
- Draft status: **게이트 R3 FAIL 정지(2026-09-09) — P-01 3라운드 소진, 출고 보류.** 소유자 결정 대기: ① 갭 인터뷰(MCP 서버 2종 스키마·전송·실패 처리 / 릴레이 서버 설계 결정 / v2 프롬프트·컨텍스트·하네스 구체 사례·참여 시기·형태) 후 보강 라운드(캡 비산입) ② 정직한 상향지원 출고 ③ 연봉 예외·Screen profile 승인 ④ 발급기관·JLPT N1 확인. docx 2종은 주입 완료(구조·문면 기계 검증 통과, Word PDF 렌더 검증은 소유자 Word 세션 점유로 보류 — troubleshooting §7). 소유자 결정 대기: ① 갭 인터뷰(에이전트 앱·Python LLM API 직접 구현·MCP 구현 세부) ② 연봉 기재 예외 재승인 ③ Screen profile AI-Product 승인 ④ 다나아데이터 회사 개요·발급기관(FP·JLPT N1)·TOEIC 만료 표기 확인

### GS네오텍 / 플랫폼개발 (풀스택) (경력, 정규직)

- Source job posting: 소유자 제공 JD 텍스트 + 지정양식 2종(`~/Downloads/[GS네오텍] 입사지원서 양식.pptx`, `~/Downloads/표준이력서_보배써치_홍길동_Sample_.docx` — 보배써치 표준이력서), 2026-09-13 수집. "첨부양식 중 선택 작성(pptx 또는 word)" — 소유자 결정: **두 양식 모두 작성**(초안 md 1개 단일 원천 → 2종 주입)
- Application deadline: ASAP (합격자 발생 시 마감)
- Target position: 플랫폼개발(풀스택) / 정규직(수습 6개월) / 직급·연봉 협의 / 서울 구로구 경인로 576. GS네오텍㈜ — 1974.07.02 설립, 대표 남기정, 매출 4,967억(2024), 직원 280여명, GS그룹 자회사 IT 서비스/솔루션 기업(JD 기재분)
- 전형: 서류 → 1차면접 → 인·적성(온라인)/2차면접 → 채용검진 → 최종합격
- Role preset: fullstack
- Required skills: Java 또는 Spring 기반 Web 개발(1년 이상) · RESTful API 이해 · Vue.js 또는 React 등 FE 프레임워크 · SQL 기본 작성·데이터 처리 · Linux 환경 · Claude Code·Codex 등 AI 개발 도구로 생산성 향상 · 학습 의지·문제 해결
- Preferred skills: SQL 성능 최적화·데이터 모델링 · AWS/GCP/NCP 클라우드 · 네트워크·클라우드 인프라 기본 · AWS IAM/STS/Cross Account · Redshift/BigQuery · Docker/Kubernetes · 대용량 데이터 처리 · 운영 자동화·모니터링
- Company / team signals: 요건이 스택 확인형(Java/Spring·REST·Vue/React·SQL·Linux)이며 AI 개발 도구 활용이 **필수 요건**. 우대는 클라우드(MSP 성격)·데이터·운영 자동화 쪽
- Candidate evidence to emphasize: 비즈36.5 Spring Boot·MyBatis·MySQL 신규 기능 3건 End-to-End(2025.11~2026.02), 삼성물산 Spring REST API·필터링/정렬 로직·DB 인덱싱·DTO 투영·Vue 3 대용량 대시보드 2.5초→1초대(2024.10~2025.04), React/TypeScript 전 경력, Node.js(Express) BFF, Linux SSH 운영, Docker 컨테이너 직접 빌드·배포·재기동 + Blue-Green 무중단 배포, AWS S3·EC2 배포·운영(구성된 환경 위), Codex·Claude Code 하네스 엔지니어링·팀 AX 리드, 국비 자바 풀스택 과정 수료(2020.03~09, 교육사항 보조 근거)
- Risks or gaps:
  - **not held (미주장)**: AWS IAM/STS/Cross Account — 발급된 키·역할로 사용만(user-attested 2026-09-13) / Redshift·BigQuery — 경험 없음(user-attested 2026-09-13) / Kubernetes — 미사용, Docker·Compose로 직접 운영(user-attested 2026-09-13)
  - [구조적] Java/Spring 실무 합계 약 11개월(삼성물산 7 + 비즈36.5 4)로 "1년 이상" 경계 — 기간 부풀리기 금지, 깊이(End-to-End·설계·인덱싱)로 설득. 내담 초기 20개월은 한국미스미 기간(시작일 2020.10 정정)이며 Java 없음 — 소유자 확인 2026-09-13(같은 세션 앞선 "Java 포함" 답변 철회)
  - AWS는 기존 스코프 가드레일("구성된 환경 위에서 배포·운영") 유지
- Company blockers (auto-FAIL): ① 위 not held 3종(IAM/STS/Cross Account·Redshift/BigQuery·Kubernetes)을 경험한 것처럼 서술 ② 근거 있는 필수 요건(Java/Spring·RESTful API·React/Vue·SQL·Linux·Claude Code/Codex) 중 하나라도 문면에서 누락
- 브리프 예외: 헤드라인을 base("프론트엔드에서 풀스택, AI까지 확장해 온 Product Engineer")에서 **B안 "Spring·React 기반 웹 서비스를 End-to-End로 구현하고, Claude Code·Codex로 개발 생산성을 끌어올리는 풀스택 개발자"** 계열로 교체(소유자 승인 2026-09-13, 이 회사 한정). "Product Engineer" 직함 → "풀스택 개발자"
- Keywords to include: Java, Spring Boot, MyBatis, RESTful API, React, Vue 3, TypeScript, MySQL, Oracle, SQL 성능 최적화, 인덱스, 데이터 모델링, Linux, Docker, Blue-Green 무중단 배포, AWS S3·EC2, Jenkins CI/CD, 대용량 데이터, 운영 자동화, 모니터링, Claude Code, Codex, 개발 생산성
- 헤드라인 문구 갱신 (2026-09-13, 소유자 "추천대로"): "끌어올리는" → **"높이는"** (적대 검증 수사 지적 수용). 배치는 핵심역량 첫 줄
- KT AICE Associate 자격 기재 (2026-09-13 소유자 지시 "KT AICE 취득했는데 (Associate) 도 추가해줘"): 이 회사 한정 **자격사항에 기재**(2026.08, KT, 100/100) — 지오영(09-09) "자격표 미기재" 결정은 지오영 한정으로 유지. 빅데이터분석기사 필기는 계속 자격표 미기재
- JLPT 표기: **N1** (소유자 확인 2026-09-13, 원본 "1급"은 구 체계 명칭)
- F-08 계층 중복 예외 (2026-09-13, 소유자 "추천대로"): 보배써치 양식이 경력사항(회사별 수행업무 명세)과 주요 project 내역(서사)을 별도 섹션으로 요구하므로 같은 사실의 명세↔서사 중복은 허용. 단 동일 문장 반복은 금지(경력사항은 명세형 명사구로)
- TOEIC 표기 (2026-09-13 소유자 결정): "TOEIC 825점 (2024.05)" 날짜만 병기해 그대로 유지 — "(만료)" 미표기, 삭제 안 함
- 연봉 기재 예외 (C-08): 헤드헌터 지정양식(보배써치) 연봉 칸에 한해 기재 — 소유자 승인 2026-09-13(grill-me Q8 "추천대로") + 현재 연봉 총 5,700만원 소유자 확인(같은 날). 기본급 분리값(5,500만원)은 재확인 없음 → 문면 미기재. 희망연봉 "면접 후 협의"
- Screen profile: Balanced   (소유자 승인 2026-09-13)
- Pass bar: 80
- 결정 변경 (2026-09-13, 소유자 "같은 결로 먼저 md를 쓰고… pptx 확장자로 굳이 쓸 필요가 없을 것 같은데"): **pptx(GS네오텍 입사지원서) 제출 취소 → 보배써치 표준이력서 docx 1종만 출고.** 원천 md는 지오영·한미글로벌 초안과 같은 결(이력서 + 자기소개서 3항목 [본인 소개 및 성격 특장점]/[직무수행 역량 및 자세]/[지원동기 및 입사 후 포부])로 작성 후 docx 주입. pptx 전용 결정(자소서 ①~⑤ 소재 배치, ② 단점·보완 서사, pptx 경력별 연봉 칸)은 폐기 — 성격은 지오영 결정대로 장점만.
- 보배써치 템플릿 작성 안내 문구(마젠타 FF00FF 런: "어필 할 핵심역량 내용을 4~6줄 기재(중요)", "(참고) …" 등) 처리 (2026-09-13 소유자 결정): **안내 문구만 삭제**, 섹션 제목·구분선·서식은 유지(F-09 범위 내 — 지원자용 작성 지시문은 값 칸이 아님)
- 게이트 이력: R1(v3) 인사 77 / 기술 68 → R2(v4) 76 / 75.5 → R3(v5) 73 / 78 FAIL, 블로커 0 — 캡 소진 시 강행 없이 보고·정지 → 소유자 "보강 1회 후 출고" → 보강(v6) 76 / 79.5 FAIL, 블로커 0 → **정직한 상향지원 출고 결정**. 잔여 감점 구조적: Java/Spring 실무 약 11개월, 백엔드 테스트 근거 없음, AI 생산성 수치 자가 기록, 인사(밀도 축소)↔기술(설계 세부 추가) 요구 상충
- 정정 이력(2026-09-13): 한국미스미 시작일 2022.06→**2020.10**(base-resume·profile·experience-bank 반영), 내담 초기 20개월 Java 없음(스마일게이트 08-04 기록 철회), "생산 공정 대시보드"=삼성물산 대시보드
- 개선 루프(소유자 지시 "좋은 점수를 받을 때까지", 2026-09-13): 4차 갭 인터뷰(JUnit 서비스·매퍼, EXPLAIN MySQL·Oracle, 판매 1.0억 GS네오텍 승인, 29건 Node.js(Express), 58→2,318 승인, 6주=과거 실제 기간) 후 이력서 L1~L5 — v8 74/75 → v9 74/77(C-01 캡) → v10 74/75.5 → **v11 74/80.5(채택)** → v12 74.5/77.5(기각). 자기소개서 SI2 84/75.5 → SI3 72/81 → SI4 74/77 → 소유자 선택 **SI3**(기술 81 PASS). 인사 74점대 정체·채점 편차 ±3 — 잔여 감점 구조적
- 제출 양식 변경 (2026-09-14 소유자 결정): **보배써치 대신 GS네오텍 입사지원서 양식을 Word로 재현해 제출** — `outputs/정다훈(만35세)_플랫폼개발_GS네오텍_입사지원서2026.docx`(5p: 기본사항·병역·학력·자격·외국어·해외연수 / 핵심역량(소유자 요청 추가 란)·경력·경험·수상 / 자소서 ①②③ / ④ / ⑤ 경력기술), 빌드 `scripts/build_gs_neotek_form_docx.py`, 렌더 확인 `outputs/gs-neotek-form-render-check.pdf`. 자소서 ①~④는 소유자 작성 원문 무수정(`outputs/gs-neotek-form-self-intro.md`, C-03 강조 볼드만 소유자 요청으로 표시), ②는 장점+단점+보완 포함. 소유자 원문 확인 요청 3건(미반영): ④ "데이터 흐름"(S-01 금지어), ① Blue-Green을 10분→2분 원인으로 병기, ① AWS "구성된 환경" 한정어 누락. 보배써치 자소서 소제목도 범위·역할형으로 교체(2026-09-14)
- Draft status: **출고 준비 완료(2026-09-13, 정직한 상향지원)** — 초안 `outputs/gs-neotek-fullstack-resume-draft.md`(v11-SI3.2, 최종 무결성 재검 must-fix 해소), companion `outputs/gs-neotek-fullstack-new-prose.md`, 제출본 `outputs/정다훈(만35세)_플랫폼개발_GS네오텍_보배써치2026.docx`(8p, 사진·성명·추천 포지션·섹션 제목/구분선 템플릿 유지, 마젠타 안내 문구만 제거), 렌더 확인 `outputs/gs-neotek-render-check.pdf`, 빌드 `scripts/build_gs_neotek_docx.py`. 면접 대비: Java/Spring 1년 요건 대비 깊이 답변, 3개월 차 이직(퇴사+신규 입사 사실 선답), 복합 인덱스 SQL·실행 계획·@Transactional 롤백·서비스 로직 내 권한 검사 이유·Blue-Green 전환 방식·2,318건 테스트 종류
