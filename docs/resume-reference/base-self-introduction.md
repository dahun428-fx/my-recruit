# Base 자기소개서 (범용 마스터) — **표준 정본**

이 문서는 소유자의 **표준 자기소개서**다. 새 회사 지원 = 이 문서에서
**편집**(재작성 아님). 2026-07-30 소유자가 항목 1·2를 base 정본으로 확정했고,
같은 날 `outputs/self-introduction-section-1-growth-and-developer-identity.md`,
`outputs/self-introduction-section-2-ai-development-standard.md` 두 파일을
이 문서로 승격했다. 2026-08-01에는 소유자가 직접 작성한 항목 3(사업 변화 →
제품 구조·운영 방식)을 자문 라운드(reviewer 사실 검증, 인사·기술 채점,
헤드헌터 평가 — 수치·표현 근거 전건 확인)를 거쳐 확정 문면 그대로 추가했다.
**문면은 소유자 확정본이므로 소유자 승인 없이 수정하지 않는다.**

## 항목 1 · 제품의 불확실성을 기술적 의사결정으로 해결하는 Product Engineer
