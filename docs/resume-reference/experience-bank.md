...
- Actions:
  ...
  - **[컴포넌트 추상화 수준 제한]** 비슷하게 생긴 UI라도 모두 공통화하지
    ...
    **[공통화 기준 갱신 — user-attested 2026-08-24, 코오롱베니트 2차 갭
    인터뷰]** 위 2026-08-12 확인을 갱신한다: 3개 서비스(비즈36.5·AI코치·
    바이오에이지) 공통 컴포넌트 레이어의 승격 기준도 "변경 이유가 같은
    UI만 공통화"(서비스마다 달라질 수 있는 UI는 개별 유지)라는 동일 원칙이
    적용됨을 소유자가 직접 확인했다. 이 갱신 이후로는 두 판단을 동일 원칙의
    적용으로 서술 가능 — 상세는 EXP-03 Notes for tailoring [공통화 기준
    갱신] 블록 참조.
...
- Actions:
  ...
  - **[매핑 레이어 확정 — user-attested 2026-08-24, 코오롱베니트 갭 인터뷰]**
  ...
  - **[테이블 설계 방식 — user-attested 2026-08-24, 코오롱베니트 2차 갭
    ...
    방식(신규 테이블 추가형)으로 설계함. 정규화 수준·인덱스 유무 등 세부는
    미확인 — 이 범위를 넘는 단정 서술 금지.
  - **[키·인덱스 설계 — user-attested 2026-08-24, 코오롱베니트 3차 갭
  ...
  - **[아키텍처 결정의 대가 — user-attested 2026-08-25, 코오롱베니트 v3
    ...
    응답과 BFF 가공 모델의 이중 관리 감수. (b) 화면 단위 점진 전환(jQuery·
  ...
  - **[WebView 호환성 사례·검증 기준 — user-attested 2026-08-25, 코오롱베니트
    ...
    동작 차이(Android와 다른 동작)로 발생한 호환성 오류 — 원인 추적 후
    수정·배포하고, 해당 화면을 E2E 회귀 시나리오에 추가 + 실기기 수동
    체크리스트로 편입해 재발 방지. 구체 iOS 버전·오류 화면 명은 미확인 —
    초과 서술 금지.
  ...
  - **[Blue-Green 전환·롤백 기제 — user-attested 2026-08-24, 코오롱베니트
    갭 인터뷰]** 신규 컨테이너 헬스체크 확인 후 트래픽을 전환하고, 이상 시
    이전 컨테이너로 롤백하는 방식. 헬스체크의 구체 조건·자동화 수준은
    미확인 — 세부 창작 금지.
...
- Evidence / links:
  ...
  - user-attested 2026-08-17 (CJ푸드빌(뚜레쥬르 APP 리빌딩) 지원 세션 갭
    ...
    대비 항목).
  - user-attested 2026-08-24 (코오롱베니트 플랫폼 개발/운영 세션 갭 인터뷰,
    문서 근거 없음) — (1) 신규 건강관리 기능 3건의 Spring Boot 백엔드는
    MyBatis 매퍼 기반 SQL 매핑임을 확인(삼성물산 프로젝트와 동일 스택).
    트랜잭션 경계·테이블 설계 기준 세부는 미확인. (2) Blue-Green 배포의
...
- Reusable keywords: B2B 플랫폼, 풀스택 내재화, Spring Boot, MySQL, MyBatis,
  ...
  비즈케어, 비즈36.5, Docker, Blue-Green 무중단 배포, 헬스체크 기반 트래픽
  전환·롤백, 반응형 웹, 미디어 쿼리,
...
- Notes for tailoring: 풀스택/플랫폼/운영 안정화 직무에 적합. 108개 페이지·
  ...
  **매핑 레이어·Blue-Green 기제 가드레일(user-attested 2026-08-24, 코오롱
  베니트 갭 인터뷰)**: 신규 건강관리 기능 3건의 서버 SQL 매핑은 MyBatis
  기반이라고만 서술 가능 — 트랜잭션 경계·테이블 설계 기준은 [확인 필요]로
...
- Actions:
  ...
  - **[비개발 현업(기획·운영) 대상 교육·표준 배포 — daewoong-history.md
    documentary, 1059행·574행. GS글로벌 FDE 지원 세션, 2026-08-22 소유자
    사용 승인]** 기획·운영팀 대상으로 요구사항·화면설계서 작성 가이드를
    교육하고, 작성 기준 및 템플릿을 수립·배포함(직무급 지원서 2026-03,
    daewoong-history.md 부록 A "5) 개발 프로세스 구축 및 조직 핵심역량
    강화" 항목, 1059행). 이와 별개로 2025-12 시점에는 화면설계서 작성
    기준을 구조화·공유해 기획자의 설계 역량 향상과 협업 효율 개선에
    기여함(월별 피드백 2025-12, "③ 인재육성" KR1, 574행). 바로 위
    Storybook 기반 "기획자와 소통하고 UI를 빠르게 확정하는" 메커니즘과는
    별개 사건 — 이쪽은 문서화된 작성 기준·템플릿의 교육/배포를 통한
    개발 요청 품질 향상 메커니즘이며, Storybook UI 조기 확정 메커니즘과
    혼동해 병합 서술하지 말 것.
    **[정량 가드레일]** 두 원문 모두 "개발 요청 품질을 높이고 협업
    커뮤니케이션 비용을 절감"/"기획자 설계 역량을 향상시키고 협업 효율
    개선"이라는 정성적 결과만 서술하며 구체 수치(절감률·시간·건수 등)는
    없다 — 정량 주장 금지, 행위·산출물(가이드 교육, 작성 기준·템플릿
    수립·배포) 수준까지만 서술할 것.
    **[선별 적재 가드레일 — daewoong-history.md 전체는 여전히 미적재
    상태]** `daewoong-history.md` 전체는 부록 C의 미해소 충돌 8건(AI코치
    사용자 수 3,493명↔약 5,000명, 매출 4.66억↔4.86억, 응답 속도 원인
    귀속(HTTP/2↔대화 이력 API), 응답/진입 속도 4계통 라벨, 비즈36.5 개편
    기간 8주↔9주, 문서 건수 48건↔130건, 연간 쉐어드 비용 절감 5,700만원의
    연봉 노출 위험, 1인당 매출 2.4억의 팀 성과 포장 리스크) 해소가
    선행되어야 은행에 적재 가능한 상태다(source-log 2026-07-29 항목 참조).
    이 두 사실(1059행·574행)은 그 충돌 8건 목록과 무관한 정성적 사실임을
    확인해 선별 적재했다 — `daewoong-history.md`의 나머지 내용(수치 포함
    항목 전반)은 여전히 미적재이며, 별도 충돌 해소 세션이 선행되어야 한다.
  - GA4/PostHog 기반 운영 데이터 대시보드 구축
    **[데이터 기반 과제 정의 — user-attested 2026-08-21, GS글로벌 FDE 지원
    세션 갭 인터뷰. 객관식 확인, 문서 근거 없음]** 위 대시보드 지표를 근거로
    개선 과제를 정의한 사례 3종을 소유자가 확인함:
    (a) 진입·이탈 지표를 근거로 화면/플로우 개선 과제를 정의;
    (b) 오류·응답 품질 지표를 근거로 품질 개선 과제를 정의;
    (c) 사용량·활성 지표를 근거로 기능 개발 우선순위를 조정.
    **[성과 수치 가드레일]** 이 세 사례는 "지표를 근거로 과제를 정의했다"는
    행위 수준까지만 확인됐다 — 개선 결과로 이어진 구체 수치(예: 이탈률
    몇 % 개선)는 미확보 상태이므로 성과 수치를 창작해 붙이지 말 것.
    **[실행 체인 확인 — user-attested 2026-08-21, GS글로벌 FDE 지원 세션
    갭 인터뷰 2차(객관식 확인), 문서 근거 없음]** 위 (a) 진입·이탈 지표로
    정의한 화면/플로우 개선 과제는 정의에서 그치지 않고 실제 실행(개선
    작업 완료)까지 이어졌음을 소유자가 확인함. 단, 개선 결과 수치(개선폭·
    이탈률 변화 등)는 여전히 미확보 상태이므로 "실행/완료했다"는 사실
    수준까지만 서술하고 성과 수치를 창작해 붙이지 말 것 — 위 [성과 수치
    가드레일]과 동일 원칙 적용.
  ...
  - **[MCP 서버 개발·연동 및 AI 챗봇 문서 연결 — user-attested 2026-08-21,
    ...
    (a) 사내 용도 MCP 서버를 직접 개발·수정함 — 용도는 ① 사내 문서/지식
    검색용, ② 테스트/검증 도구 호출용 2종(용도 확정, 아래 [MCP 서버 용도
    확정] 참조);
    (b) Claude Code/Codex 개발 환경에 MCP 도구(서버)를 연동해 AI 에이전트가
    그 도구를 호출할 수 있도록 구성함;
    (c) 사내 문서(docs)를 AI/챗봇이 참조할 수 있도록 연결하는 작업을 수행함 —
    연결 방식 세부는 [확인 필요];
    (d) 장애·오류 트러블슈팅에 AI를 결합해 절차화함(팀 절차/문서화 맥락).
    **[MCP 서버 용도 확정 — user-attested 2026-08-21, GS글로벌 FDE 지원
    세션 갭 인터뷰 2차(객관식 확인), 문서 근거 없음]** 위 (a)에서 직접
    개발·수정한 MCP 서버의 용도를 소유자가 2종으로 확정함: ① 사내 문서/
    지식 검색용, ② 테스트/검증 도구 호출용. 두 용도 모두 명시해 서술
    가능(기존 "사내 용도 수준까지만 사용 가능" 제한 해제). 단, 두 용도 외
    다른 대상·용도(예: 배포·모니터링 자동화용)가 있었다는 서술은 하지 않는다.
    **[확대 해석 금지 가드레일]** 이 4가지는 도구 호출 환경 구성·MCP 서버
    개발·트러블슈팅 절차화라는 사실 수준에 한정된다 — "AI 에이전트 프레임워크
    전문성", "멀티 에이전트 오케스트레이션 설계" 등 상위 역량 주장으로 확대
    서술하지 말 것.
  - **[운영 장애 사례 — user-attested 2026-08-24]** 운영 중 특정 장애 대응
    대표 사례는 소유자가 "특정 사례 없음"으로 확인 — 장애 대응 사례 서술
    금지(기존 "장애·오류 분석 AI 활용 절차 수립"·"오류 4유형 복구 전략"
    문면까지만 사용).
  - **[운영 문의·이슈 대응 흐름 — user-attested 2026-08-24, 코오롱베니트
    3차 갭 인터뷰]** 운영 중 운영팀·고객사 문의와 버그 리포트를 받아 원인을
    추적해 수정·배포하는 흐름을 수행했고, WebView·앱 플랫폼별 호환성 이슈
    대응 경험도 있음(둘 다 소유자 확인). 위 [운영 장애 사례] 가드와의 관계:
    장애급 인시던트 서사는 여전히 금지, 문의·버그 대응 흐름과 호환성 이슈
    대응까지만 서술 가능. 구체 사례(어떤 문의·어떤 이슈)는 미특정 — 개별
    사례 창작 금지.
...
- Evidence / links:
  ...
  - user-attested 2026-08-21 (GS글로벌 FDE 지원 세션 갭 인터뷰, 객관식 확인) —
    (1) 사내 용도 MCP 서버 직접 개발·수정, Claude Code/Codex 개발 환경에
    MCP 도구(서버) 연동해 AI 에이전트가 도구를 호출하도록 구성, 사내 문서를
    AI/챗봇이 참조하도록 연결, 장애·오류 트러블슈팅에 AI를 결합해 절차화
    — 4건 모두 확인. 대상·연결 방식 세부는 [확인 필요]. (2) GA4/PostHog
    운영 데이터 대시보드 지표를 근거로 개선 과제를 정의한 사례 3종(진입·
    이탈 지표→화면/플로우 개선, 오류·응답 품질 지표→품질 개선, 사용량·
    활성 지표→기능 개발 우선순위 조정) 확인 — 과제 정의 행위까지만 확인됐고
    개선 결과 수치는 미확보. 문서 근거 없음, pending documentary source.
  - user-attested 2026-08-21 (GS글로벌 FDE 지원 세션 갭 인터뷰 2차, 객관식
    확인) — (1) 위 (1)의 MCP 서버 대상·용도 [확인 필요]를 해소: 직접
    개발·수정한 MCP 서버 용도는 ① 사내 문서/지식 검색용, ② 테스트/검증
    도구 호출용 2종(사내 문서 연결 방식 세부는 여전히 [확인 필요]). (2) 위
    (2)의 진입·이탈 지표 기반 화면/플로우 개선 과제가 정의에서 그치지 않고
    실제 실행까지 완료됐음을 확인 — 개선 결과 수치는 여전히 미확보.
    문서 근거 없음, pending documentary source.
  - `daewoong-history.md` documentary, 1059행 — "기획·운영팀 대상
    요구사항·화면설계서 작성 가이드를 교육하고 작성 기준 및 템플릿을
    수립·배포하여, 개발 요청 품질을 높이고 협업 커뮤니케이션 비용을
    절감"(직무급 지원서 2026-03, 부록 A). 574행 — "화면설계서 작성 기준을
    구조화·공유하여 기획자 설계 역량을 향상시키고 협업 효율 개선"(월별
    피드백 2025-12, ③ 인재육성 KR1). 2026-08-22 GS글로벌 FDE 지원 세션,
    소유자 선별 적재 승인 — `daewoong-history.md` 전체(부록 C 충돌 8건
    미해소)와 별개로 이 2건만 선별 적재.
  - user-attested 2026-08-24 (코오롱베니트 플랫폼 개발/운영 세션 갭 인터뷰) —
    운영 중 특정 장애 대응 대표 사례를 소유자가 "특정 사례 없음"으로 확인.
...
- Reusable keywords: Playwright, Storybook, FE AX, SOP, 품질 자동화, 운영 데이터
  ...
  MCP, MCP 서버 개발, 사내 문서/지식 검색용 MCP 서버, 테스트/검증 도구
...
- Notes for tailoring: 생산성/품질/AI 개발 프로세스 개선을 강조할 때 사용.
  ...
  **[공통화 기준 갱신 — user-attested 2026-08-24, 코오롱베니트 2차 갭
  인터뷰]** 위 2026-08-12 "공통 컴포넌트 레이어 판정 기준 가드레일"(EXP-01
  기준 적용 근거 없음)을 갱신한다: 3개 서비스(비즈36.5·AI코치·바이오에이지)
  공통 컴포넌트 레이어의 승격 기준은 "변경 이유가 같은 UI만 공통화"(서비스
  마다 달라질 수 있는 UI는 개별 유지)이며, EXP-01의 챗봇 내부 한정 기준과
  동일 원칙이 이 공통 레이어에도 적용됨을 소유자가 직접 확인했다. 이 확인
  이후로는 두 판단을 별개 기준으로 서술하는 대신 동일 원칙의 적용으로
  서술 가능하다 — 위 2026-08-12 블록은 이 갱신 이전 시점의 기록으로 보존.
  ...
  **MCP·AI 에이전트 도구 연동 가드레일(user-attested 2026-08-21, GS글로벌
  FDE 지원 세션 갭 인터뷰, 2차 갭 인터뷰로 용도 확정)**: 사내 용도 MCP
  서버 개발·수정(용도: ① 사내 문서/지식 검색용, ② 테스트/검증 도구
  호출용 — 이 2종은 명시 가능), Claude Code/Codex에 MCP 도구 연동, 사내
  문서(docs)를 AI/챗봇에 연결, 트러블슈팅+AI 결합 절차화 4건은 서술
  가능하나 "AI 에이전트 프레임워크 전문성"·"멀티 에이전트 오케스트레이션
  설계" 등 상위 주장으로 확대하지 말 것 — 도구 호출 환경 구성·MCP 서버
  개발이라는 사실 수준으로 한정. 사내 문서 연결 방식 세부는 여전히
  [확인 필요] — 세부 확인 전 외부 문서에서는 방식을 구체화하지 말 것
  (MCP 서버 용도는 위 2종까지 사용 가능, 기존 "사내 용도 수준까지만" 제한
  해제).
  **데이터 기반 과제 정의 가드레일(user-attested 2026-08-21, 같은 세션,
  2차 갭 인터뷰로 실행 사실 추가)**: GA4/PostHog 대시보드 지표를 근거로
  (a) 화면/플로우 개선 과제, (b) 품질 개선 과제, (c) 기능 개발 우선순위
  조정 과제를 정의한 사실은 서술 가능하며, (a) 화면/플로우 개선 과제는
  실제 실행(개선 작업 완료)까지 이어졌다고 서술 가능하다(2차 갭 인터뷰
  확인). 단, 개선 결과로 이어진 구체 수치(이탈률·오류율 개선폭 등)는
  미확보이므로 성과 수치를 창작해 붙이지 말 것 — "과제를 정의했다"/
  "실행을 완료했다" 수준까지만 주장한다.
  **비개발 현업 교육·표준 배포 가드레일(daewoong-history.md documentary,
  1059행·574행, 2026-08-22 GS글로벌 FDE 지원 세션 선별 적재)**: "기획·
  운영팀 대상 요구사항·화면설계서 작성 가이드 교육 + 작성 기준/템플릿
  수립·배포"(1059행)와 "화면설계서 작성 기준 구조화·공유"(574행)는 문서
  근거가 있는 확정 사실이지만 정량 수치가 없다 — "개발 요청 품질 향상",
  "협업 커뮤니케이션 비용 절감", "기획자 설계 역량 향상", "협업 효율
  개선" 등 원문 그대로의 정성적 표현까지만 사용하고 %, 시간 등 수치를
  창작해 붙이지 말 것. 위 Storybook 기반 커뮤니케이션 비용 절감(리드타임
  2일→1일)과는 별개 메커니즘이므로 병합 서술하지 말 것. `daewoong-history.md`
  전체는 부록 C 충돌 8건이 미해소 상태이므로 이 2건 외 나머지 내용을
  근거 없이 인용하지 말 것.
  **운영 장애 사례 가드레일(user-attested 2026-08-24, 코오롱베니트 갭
  인터뷰)**: 운영 중 특정 장애 대응 대표 사례는 소유자가 "특정 사례
  없음"으로 확인했다 — 구체 장애 사건(무엇이 터졌고 어떻게 복구했는지)을
...
- Metrics:
  ...
  - **[측정 도구 확정 — user-attested 2026-08-24, 코오롱베니트 갭 인터뷰]**
    체류 시간 32% 증가와 평균 로딩 속도 약 50% 개선의 측정 도구는
    GA(Google Analytics). 측정 기간·페이지 범위는 여전히 미확인 — 문면
    스코프는 "(GA 기준)"까지만 병기 가능.
...
- Notes for tailoring: 커머스/프론트엔드 성능 개선 사례로 활용. **아키텍처
  ...
  **체류 시간 가드레일(user-attested 2026-07-25; 측정 도구 확정
  ...
  32% 증가"는 기능 고도화 프로젝트의 귀속 수치이며, 측정 도구는 GA(Google
  Analytics)로 확정됐다(평균 로딩 약 50% 개선도 동일 도구). 측정 기간·
  페이지 범위는 여전히 [확인 필요] — 외부 문서에서 사용할 때는 "(GA
  기준)"까지만 병기하고 기간·범위를 창작하지 말 것. 다른 프로젝트
  (유지보수·성능 개선, Next.js 전환)의 성과와 혼용 금지.
...
- Actions:
  ...
  - **[BFF 담당 범위 정밀화 — user-attested 2026-08-24, 코오롱베니트 2차
    갭 인터뷰]** Node.js 집계·중계 API는 구조 설계와 주요 엔드포인트
    구현을 본인이 주도했고 팀이 리뷰·보완함(기존 "팀 공동" 기록의 정밀화
    — 소유자 직접 확인). 타임아웃·부분 실패 처리·캐싱 등 운영 세부는
    미확인 — 창작 금지.
  - **[BFF 책임 경계 — user-attested 2026-08-25, 코오롱베니트 v3 미니 갭
    인터뷰]** 조회 조합 외 일부 가공·캐싱·권한 확인도 BFF에서 처리하고,
    비즈니스 로직·데이터 소유는 Spring Boot에 유지 — 소유자 직접 확인.
    권한 확인의 구체 방식(토큰 검증·역할 필터 등)은 미확인 — 창작 금지.
  - **[BFF 실패 처리 — user-attested 2026-08-24, 코오롱베니트 3차 갭
    인터뷰]** 하위 백엔드 호출에 타임아웃을 두고 실패 시 오류 응답을
    반환, 프론트 화면에서 오류 상태로 처리하는 방식. 부분 응답·캐싱·재시도
    정책은 없음 또는 미확인 — 서술 금지.
  - AI 챗봇(Python) 서버의 프롬프트 구성·응답 후처리(Markdown/표/링크 변환 등) 로직
    일부를 직접 수정·기여
    **[응답 후처리 구체화 — user-attested 2026-08-21, GS글로벌 FDE 지원
    세션 갭 인터뷰 2차(객관식 확인), 문서 근거 없음]** 위 응답 후처리 로직
    중 본인이 직접 수정한 대상은 ① LLM 응답의 표/마크다운 깨짐 수정, ②
    링크/버튼 변환 로직 수정 2종으로 구체화됨. 기존 "참여·기여" 톤은 유지
    — "설계·주도"로 확대 서술 금지.
  ...
  - **[Langchain 활용 파인튜닝 경험]** Langchain을 활용한 LLM 파인튜닝 과정에 경험함.
    ...
    (user-attested 2026-07-24, 소유자 갭 인터뷰)
  - ~~**[Python pandas 데이터 처리]** Python pandas 라이브러리로 데이터 처리 경험
    있음. 소유자 자평 "그렇게 깊이는 깊지 않음" — 기초·경험 수준이며 데이터
    엔지니어링 수준의 전문성 주장 금지.~~ — 2026-08-21 소유자 철회, 외부 문서
    사용 금지. "pandas 전처리 과정은 하지 않았다고 하자"(소유자 발화) — 이력서·
    자기소개서·포트폴리오 등 어떤 문면에도 이 사실을 인용하지 말 것.
    (원 출처: user-attested 2026-07-24, 소유자 갭 인터뷰; 철회: user-attested
    2026-08-21, GS글로벌 FDE 지원 세션 갭 인터뷰)
- Technologies: Node.js, Express, TypeScript, Python, LangChain, VectorDB, RAG,
  ~~pandas~~ (2026-08-21 소유자 철회, 외부 문서 사용 금지), REST API, SSE
- Result:
...
- Evidence / links:
  ...
  - user-attested 2026-07-24 (소유자 갭 인터뷰, GS리테일 AX 지원 계기) —
    VectorDB·RAG 구현 참여, Langchain 활용 파인튜닝 경험, ~~Python pandas 데이터
    처리 경험~~ 확인. 소유자 자평 "깊이는 깊지 않음" — 전부 참여·경험 범위로 한정.
    문서 근거 미확보 (user-attested only, 면접 대비 needs_confirmation 성격).
  - user-attested 2026-08-21 (GS글로벌 FDE 지원 세션 갭 인터뷰) — 위 pandas
    데이터 처리 경험을 소유자가 철회("pandas 전처리 과정은 하지 않았다고
    하자"). Actions·Technologies의 해당 기재를 취소선 처리하고 외부 문서
    사용을 금지함.
  - user-attested 2026-08-21 (GS글로벌 FDE 지원 세션 갭 인터뷰 2차, 객관식
    확인) — AI 챗봇(Python) 응답 후처리 로직 중 본인이 직접 수정한 대상을
    ① LLM 응답의 표/마크다운 깨짐 수정, ② 링크/버튼 변환 로직 수정 2종으로
    구체화. 문서 근거 없음, pending documentary source.
  - user-attested 2026-08-24 (코오롱베니트 2차 갭 인터뷰) — Node.js
    집계·중계 API의 담당 범위를 정밀화: 구조 설계·주요 엔드포인트 구현은
    본인 주도, 팀은 리뷰·보완 역할이었음을 소유자가 직접 확인. 기존
    "팀 공동" 기록을 대체하는 것이 아니라 정밀화하는 것 — 운영 세부(타임아웃·
...
- Reusable keywords: Node.js, Express, BFF, TypeScript 스택 통일, 비동기 I/O,
  ...
  (~~pandas, 데이터 처리~~ — 2026-08-21 소유자 철회, 키워드로 사용 금지)
- Notes for tailoring: Node/Python이 필수·우대인 포지션(두산로보틱스 Fullstack 등)에서
  ...
  **[AI/데이터 기여 범위 가드레일 — user-attested 2026-07-24]**: VectorDB·RAG 구현
  참여 및 Langchain 파인튜닝은 소유자가 "깊이는 깊지 않음"으로 자평한 참여·경험
  수준이다. 이력서·자기소개서에서 이 항목들을 인용할 때는 "참여", "활용 경험",
  "기여" 표현만 허용하며 "주도", "설계", "구축", "전문" 등 심화 역량으로 확대하지
  않는다. 문서 근거가 없으므로 고부담 외부 문서에서는 소유자에게 뒷받침 자료
  확인을 요청하는 것이 안전하다.
  **[pandas 철회 가드레일 — user-attested 2026-08-21, GS글로벌 FDE 지원 세션
  갭 인터뷰]**: "Python pandas 데이터 처리 경험"은 소유자가 명시적으로 철회한
  사실이다("pandas 전처리 과정은 하지 않았다고 하자"). 이 항목은 삭제하지 않고
  Actions·Technologies·Reusable keywords에 취소선(~~)으로 보존하되, 어떤
  외부 문서(이력서·자기소개서·포트폴리오)에도 인용하지 말 것 — 위반 시 사실
  왜곡이 된다.
  **[응답 후처리 구체화 가드레일 — user-attested 2026-08-21, GS글로벌 FDE
  지원 세션 갭 인터뷰 2차]**: "응답 후처리" 기여를 구체화할 때는 ① LLM
  응답의 표/마크다운 깨짐 수정, ② 링크/버튼 변환 로직 수정 2종까지 명시
  가능하다(소유자 확인). 단, 위 [AI/데이터 기여 범위 가드레일]의 "참여·
  기여" 톤 제한은 그대로 유지 — 두 항목을 "설계·주도·독립 구축"으로
  확대 서술하지 말 것.
  **[BFF 담당 범위 정밀화 가드레일 — user-attested 2026-08-24, 코오롱베니트
  2차 갭 인터뷰]**: Node.js 집계·중계 API에 한해 "본인 주도 구조 설계 +
  주요 엔드포인트 구현, 팀 리뷰·보완"으로 서술 가능하다(기존 "팀 공동"
  일괄 표기의 정밀화). AI 챗봇(Python) 프롬프트·후처리 로직 기여는 이
  정밀화 대상이 아니며 위 [AI/데이터 기여 범위 가드레일]의 "참여·기여"
  톤 제한을 그대로 유지한다 — 두 갭 인터뷰의 확인 범위를 혼동하지 말 것.
