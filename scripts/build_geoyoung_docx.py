...
PPR_RES    = PPR(before=0, after=20, left=420, hanging=180)            # → 성과
PPR_RES_LAST = PPR(before=0, after=90, left=420, hanging=180)         # → 성과(마지막, 실행과 간격)
PPR_EXEC   = PPR(before=0, after=20, left=460, hanging=200)            # - 실행
...
for bi, blk in enumerate(C.BLOCKS):
    blocks += para(PPR_HDR, blk['header'], bold_all=True)
    ...
    for pj in blk['projects']:
        ...
        for ri, r in enumerate(pj['results']):
            last = (ri == len(pj['results']) - 1)
            blocks += para(PPR_RES_LAST if last else PPR_RES, r, sz=19)
        for e in pj['execs']:
            blocks += para(PPR_EXEC, e, sz=19)
