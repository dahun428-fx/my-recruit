"""Render smilegate-full-resume-with-self-intro.html (이력서 9p + 자기소개서 3p)
to PDF via headless Chrome --print-to-pdf.
Edit the HTML/CSS by hand, then re-run to refresh the PDF.
"""
...
INPUT_HTML = ROOT / "outputs" / "smilegate-full-resume-with-self-intro.html"
OUTPUT_PDF = ROOT / "outputs" / "smilegate-full-resume-with-self-intro.pdf"
CHROME_CANDIDATES = [
...
def render_pdf(html_path: Path, pdf_path: Path) -> None:
    chrome = find_chrome()
    with tempfile.TemporaryDirectory(prefix="smilegate-si-chrome-") as user_dir:
