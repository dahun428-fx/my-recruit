"""Render the Coupang Eats resume PDF from the page-structured HTML.

The résumé is a hand-authored, page-structured document
(``outputs/coupang-eats-full-resume.html`` + ``.css``) that reuses the
...
blocks — matching ``doosan-robotics-full-resume.html``.
...
INPUT_HTML = ROOT / "outputs" / "coupang-eats-full-resume.html"
OUTPUT_PDF = ROOT / "outputs" / "coupang-eats-full-resume.pdf"
...
def render_pdf(html_path: Path, pdf_path: Path) -> None:
    chrome = find_chrome()
    with tempfile.TemporaryDirectory(prefix="coupang-chrome-") as user_dir:
        cmd = [
            chrome,
            "--headless=new",
            "--disable-gpu",
