...
"""피플렙 지정양식 이력서 docx에 hanmi_content.py(= 초안 md 파싱) 값을 주입.
...
실행: /usr/bin/python3 scripts/build_hanmi_docx.py <템플릿_이력서.docx> [출력접미사]
"""
import os, re, sys, zipfile, difflib
from xml.etree import ElementTree as ET
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import hanmi_content as C

REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
TPL_RESUME = sys.argv[1]
# argv[3]: 출력 접미사(선택). 기존 제출본을 덮어쓰지 않고 새 파일로 내보낼 때 사용.
#   예) /usr/bin/python3 scripts/build_hanmi_docx.py <이력서템플릿> _v2
SUFFIX = sys.argv[2] if len(sys.argv) > 2 else ''
OUT_RESUME = os.path.join(REPO, 'outputs', f'정다훈(만{C.AGE}세)_AI개발_한미글로벌_(주)피플렙_임덕빈2026{SUFFIX}.docx')
...
prose_ppr = ('<w:pPr><w:wordWrap/>' + _AUTOSPACE_OFF + '<w:spacing w:before="0" w:after="70" w:line="264" w:lineRule="auto"/>'
             '<w:ind w:left="120" w:right="120"/><w:jc w:val="left"/>'
             '<w:textAlignment w:val="baseline"/></w:pPr>')
subhead_ppr = ('<w:pPr><w:keepNext/><w:wordWrap/>' + _AUTOSPACE_OFF + '<w:spacing w:before="120" w:after="40" w:line="264" w:lineRule="auto"/>'
...
# ---- 본문 말미 빈 문단 제거 (마지막 빈 페이지 방지) --------------------------------
# 템플릿 잔여 빈 문단 2개가 sectPr 앞에 남아 8페이지째 빈 장을 만든다. 텍스트가 없는
# 문단만 제거하므로 문면 변화 없음(F-09 무변형 범위: 본문 텍스트·서식 불변).
_sect = doc.rfind('<w:sectPr')
_removed = 0
while True:
    _m = None
    for _mm in re.finditer(r'<w:p[ >](?:(?!</w:p>).)*?</w:p>', doc[:_sect], re.S):
        _m = _mm
    if _m is None:
        break
    if re.findall(r'<w:t[^>]*>([^<]*)</w:t>', _m.group(0)):
        break                      # 텍스트가 있는 문단을 만나면 중단
    doc = doc[:_m.start()] + doc[_m.end():]
    _sect = doc.rfind('<w:sectPr')
    _removed += 1
print(f'9-1. 말미 빈 문단 {_removed}개 제거')
...
open(os.path.join(REPO, 'outputs', 'hanmi-docx-textdiff.txt'), 'w').write('\n'.join(diff))
print(f'   텍스트 diff {len(diff)}줄 → outputs/hanmi-docx-textdiff.txt')
