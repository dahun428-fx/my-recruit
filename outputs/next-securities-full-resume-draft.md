# 정다훈 이력서 및 경력기술서 — 넥스트증권 웹 프론트엔드 개발자 (경력) 지원용
...
→ **6인 팀 리드로 전면 전환 완수**, 평균 로딩 약 50% 개선·페이지 접근 8초 → 2초 / 대웅 플랫폼 **108개 페이지 9주 단독 내재화**
...
목차: 1. 대웅제약 AI 건강검진 챗봇 제품화 / 2. 대웅제약 임직원 건강검진 통합플랫폼 구축 / 3. 대웅제약 팀 AX 전환 리드 / 4. 삼성물산 대용량 데이터 플랫폼·모바일 고도화 / 5. 한국미스미 글로벌 B2B 커머스 Next.js 전면 전환
...
AI 건강검진 챗봇을 10주 내에 실서비스로 올려야 했습니다. MVP 정의와 PoC 검증을 거친 뒤 프론트엔드 전체를 구현해야 했고, LLM 출력은 형식과 품질이 매번 달라 화면에서 무엇을 보장할 수 있는지부터 정의해야 했습니다.
...
- **품질 체계**
  Vitest·Playwright E2E로 **반복 QA 3시간 → 30분**, PostHog·Web Vitals 기반 사용성 4.18점·만족도 4.06점 확보
...
- **통합플랫폼 도약**
  2019년 레거시를 React 기반 임직원 건강검진 통합플랫폼으로 재구축하고 UI/UX 전면 개편을 주도
- **단독 풀스택 전환**
...
- **검증·배포 자동화**
  수동 FTP 배포를 Jenkins·Docker 파이프라인으로 전환해 **배포 리드타임 10분 → 2분(약 80%) 단축**, 운영 서버는 Blue-Green 방식으로 서비스 중단 없는 릴리스 확보
