# 티빙(TVING) Frontend Engineer — Web Core Development 이력서
...
6년차 프론트엔드 개발자. React·Next.js 웹·웹뷰 서비스의 성능 최적화부터
AI 기반 개발 프로세스 개선까지 함께 다루는 Frontend Engineer입니다. 미디어 쿼리
기반 반응형 웹, WebView 호환성 검증, GitLab MR·AI 코드 리뷰를 실무로 운용했고,
AWS S3·EC2 배포 파이프라인을 활용·운영하며 FE 역할 범위를 API·배포까지 이해하는
방식으로 확장해 왔습니다. 대규모 트래픽 서비스의 웹·웹뷰 사용자 여정을 성능과
품질 자동화로 뒷받침하는 환경에서 이 역량을 더 깊이 발전시키고 싶습니다.
...
모바일 WebView · iOS/Android 호환성 검증 · 배포 자동화 · 풀스택 End-to-End

→ **108개 페이지 9주 단독 내재화**, 미디어 쿼리 반응형·WebView 호환성 검증 / 매출 1.0억 기여

### ④ AI 챗봇 제품화 · 비즈니스 임팩트

FE 아키텍처 분석·설계·구현 전 과정 주도 · SaaS 확장 구조 · SSE 실시간 스트리밍
...
- 기간: 2025.07 ~ 재직중
- 직무: Frontend Engineer / 비즈36.5·AI코치·바이오에이지 신규 개발 및 유지보수 / 팀 AX 전환 리드
...
- **AX 전환 리드** (Claude Code·Codex·GitLab MR·AI 코드 리뷰·하네스 엔지니어링)
- 바이오에이지 서비스 **AWS S3·EC2 배포 파이프라인 활용·운영**

### 내담씨앤씨 SI사업부

- 기간: 2020.10 ~ 2025.06
- 직무: 웹 프론트엔드 개발자 / 삼성물산·한국미스미 프로젝트
...
목차: 1. 대웅제약 AI 건강검진 챗봇 제품화 / 2. 대웅제약 팀 AX 전환 리드 — AI 개발 프로세스 개선 / 3. 대웅제약 임직원 건강검진 통합플랫폼 구축 / 4. 한국미스미 글로벌 B2B 커머스 Next.js 전면 전환 / 5. 삼성물산 대용량 데이터 플랫폼·모바일 고도화
...
- **품질 체계**
  Vitest 약 2,600케이스·Playwright E2E로 **반복 QA 3시간 → 30분**, PostHog·Web Vitals 기반 사용성 4.18점·만족도 4.06점(2차 PoC 검증 기준) 확보
...
### 2. 대웅제약 | 팀 AX 전환 리드 — AI 개발 프로세스 개선

- 기간: 2026.01 ~ 2026.06
- [주요 업무]: 팀의 AI 활용 개발 방식 전반을 설계·리드 — AI 생성 코드 검증을 위한 하네스 엔지니어링 구축, FE AX 개발 표준 단독 설계, GitLab MR 기반 AI 코드 리뷰 도입, 품질·운영 자동화와 팀 지식 자산화 주도
...
- 기간: 2025.11 ~ 2026.02
- [주요 업무]: 2019년 레거시 검진 서비스를 React 기반 임직원 건강검진 통합플랫폼으로 현대화 — 서비스·DB 구조 분석부터 점진 전환 설계, MySQL·Spring Boot·REST API·화면 End-to-End 개발, 배포 자동화, WebView 호환성 검증까지 풀스택 총괄
...
- **배포 자동화**
  FileZilla 수동 이관에 의존하던 배포를 **개발 서버 Jenkins 파이프라인**과 운영 서버 **Docker 기반 Blue-Green 무중단 배포**로 전환, **WebView 호환성 검증 기준 수립**, 반응형 웹(미디어 쿼리 기반 PC/모바일 대응) 적용
...
- **글로벌 요구 대응·관측성 운영**
  i18next 기반 **영·중·일 다국어 구조와 SEO 대응**, **Lighthouse·GA·Adobe Analytics·Datadog으로 성능·SEO·오류 지표화**, **Vercel 배포 자동화**, 미디어 쿼리 기반 PC/모바일 반응형 웹 적용
...
- **성능 개선**
  유지보수·성능 개선 프로젝트에서 Lighthouse 병목 분석과 리소스 최적화로 **페이지 접근 시간 8초 → 2초 단축** / 월 방문자 약 112만(GA·Adobe Analytics 기준) 서비스
- **사용자 경험 개선**
...
### 5. 삼성물산 | 대용량 데이터 플랫폼·모바일 서비스 고도화
