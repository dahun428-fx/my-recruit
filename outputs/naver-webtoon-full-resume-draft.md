# 정다훈 이력서 및 경력기술서 — 네이버웹툰 프론트엔드 개발자 (글로벌 웹 플랫폼) 지원용
...
6년차 프론트엔드 개발자입니다. React·Next.js·TypeScript로 레거시를 전환하고 SSR·CSR 렌더링 경계를 직접 판단해 온 엔지니어이며, 글로벌 B2B 커머스에서 PHP·jQuery 코드베이스를 Next.js로 전면 전환해 평균 로딩 약 50% 개선을 이끌었습니다. 공통 컴포넌트·Config-Driven UI 구조로 복잡한 비즈니스 요구사항을 프론트엔드 모델로 번역합니다. 모놀리식 BE–FE 분리 전략의 일환으로 React 전용 집계·프록시 BFF를 Node.js(Express)로 신규 구축했습니다.
...
일본 본사 개발자와 일본어로 직접 협업하며 다국어 글로벌 커머스를 전환해 온 경험을, 글로벌 웹 플랫폼과 일본 현지 조직 협업에 바로 잇고 싶습니다. AI 도구를 팀의 개발 표준으로 정착시킨 방식 그대로 AX 과제 발굴에도 함께하겠습니다.

## 핵심 성과

### ① 대규모 레거시 전환

기술 리드 · 글로벌 B2B 커머스 · PHP·jQuery → Next.js·TypeScript · 렌더링 성능·SEO 대응

→ **6인 팀 리드로 전면 전환 완수**, 평균 로딩 약 50% 개선·페이지 접근 8초 →
2초 / 대웅 플랫폼 **108개 페이지 9주 단독 내재화**, 조직 매출 1.0억 기여
...
검색 응답 5초→1초 · 대시보드 2.5초→1초대 · 구간 분리 측정·FE·BE 양단 개입
...
- **대용량 데이터 처리 및 처리 소요 시간 단축** (대시보드 렌더링 2.5초 → 1초대 · 수만 건 검색 응답 5초 → 1초)
...
- **품질 체계**
  Vitest 약 2,600케이스·Playwright E2E로 **반복 QA 3시간 → 30분**, PostHog·Web Vitals 기반 사용성 4.18점·만족도 4.06점 확보 (2차 PoC 검증 기준)
...
- **통합플랫폼 도약**
  2019년 레거시를 React 기반 임직원 건강검진 통합플랫폼으로 재구축하고 UI/UX 전면 개편을 주도, 대웅제약 대상 **신규 판매(조직 매출 1.0억 원)에 기여**
- **단독 풀스택 전환**
...
- **공통 컴포넌트 표준화**
  "변경 이유가 같은 것만 공통화"하는 기준을 적용해 과공통화(거대 컴포넌트)와 공통화 전무(반복 수정)의 트레이드오프를 조정, **Storybook 스토리 210개**로 재사용 컴포넌트를 카탈로그화하고 기획자와 UI를 조기 확정해 신규 화면 조립을 공통 컴포넌트 기반으로 전환
- **운영 확인 자동화**
